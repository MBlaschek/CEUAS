...
def read_bufr_stn_meta(varno,bufrfile):
    ...
    with open(bufrfile) as f:
        ...
        for varno in vlog.keys():
            ...
            station_configuration['observed_variables'].append(int(varno)) 
        a = open('/raid8/srvx1/federico/GitHub/CEUAS_master/CEUAS/meta/inventory0/code/bufr_stationIds_filename.dat' ,'a+')
        a.write(str(station_configuration['primary_id']) + '_' + str(station_configuration['secondary_id']) + '_' + bufrfile + '\n'  )
...
def read_rda_meta(ncfile):
    ...
    a = open('/raid8/srvx1/federico/GitHub/CEUAS_master/CEUAS/meta/inventory0/code/ncar_stationIds_filename.dat' ,'a+')
    a.write(str(station_conf['primary_id']) + '_' + str(station_conf['secondary_id']) + '_' + original_file + '\n'  )
...
#                print(len(li))
        ...
        f = '/raid8/srvx1/federico/GitHub/CEUAS_master/CEUAS/meta/inventory0/data/tables/vola_legacy_report.txt'
        ...
        for odir in dbs: 
            if '3188' in odir:
                ...
                flist = glob.glob( '/raid60/scratch/leo/scratch/era5/odbs/3188' )
                ...
                #transunified=p.map(func,flist)
                
            #with open(dpath+odir+'/orphans.csv','w'):
             #   pass
            elif '1759' in odir:
                 #flist=glob.glob(odir+'/'+'era5.*.bfr')
                 flist = os.listdir('/raid60/scratch/leo/scratch/era5/odbs/1759' )
                 flist = [ '/raid60/scratch/leo/scratch/era5/odbs/1759' + '/' + f for f in flist if '.conv.' in f and '.nc' not in f]
                 transunified=list(p.map(func,flist))

            elif '1761' in odir:
                 #flist=glob.glob(odir+'/'+'era5.*.bfr')
                 flist = os.listdir('/raid60/scratch/leo/scratch/era5/odbs/1761' )                
                 flist = [ '/raid60/scratch/leo/scratch/era5/odbs/1761' + '/' + f for f in flist if '.conv.' in f and '.nc' not in f]
                 #for f in flist:
                  #  a =  odb_cdm(ci,cdm,cdmd,unified,vola,chuan,igrainv,wbaninv,trans,f)
                ...
                os.system('rm /raid8/srvx1/federico/GitHub/CEUAS_master/CEUAS/meta/inventory0/code/bufr_stationIds_filename.dat')
                a = open('/raid8/srvx1/federico/GitHub/CEUAS_master/CEUAS/meta/inventory0/code/bufr_stationIds_filename.dat', 'a+')
                #flist=glob.glob(odir+'/'+'era5.*.bfr')
                flist = glob.glob( '/raid60/scratch/leo/scratch/era5/odbs/ai_bfr/era5.*.bfr' )
                #flist = ['/raid60/scratch/leo/scratch/era5/odbs/ai_bfr/era5.1094.bfr']
                transunified=list(p.map(bfunc,flist))
                
            elif 'rda' in odir:
                os.system('rm /raid8/srvx1/federico/GitHub/CEUAS_master/CEUAS/meta/inventory0/code/ncar_stationIds_filename.dat')
                a = open('/raid8/srvx1/federico/GitHub/CEUAS_master/CEUAS/meta/inventory0/code/ncar_stationIds_filename.dat', 'a+')
