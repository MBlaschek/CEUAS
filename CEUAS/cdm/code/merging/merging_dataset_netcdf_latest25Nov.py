...
class Merger():
      ...
      def MergeAllData(self, dictionary_data = ''):
            ...
            for dt in date_times: # loop over all the possible date_times 
                  ...
                  print('Analize the date_time number ', str(date_times.index(dt)) , ' ' , dt ,  ' ', datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S') )
                  ...
                  for k in dictionary_data[dt].keys() :
                        for v in all_var:
                              if v in dictionary_data[dt][k].keys():
                                    try:          
                                          pressures = dictionary_data[dt][k][v]
                                          for p in pressures:        
                                                if p < 0:  #to get rid of some negative geopotentials
                                                       continue 
                                                if p not in all_press and p != np.nan:
                                                      all_press.append(float(p) )
                                    except:
                                          pass 
                                    
                  all_press.sort(reverse = True)
                  
                  """ Case where there are valid pressure levels """   
                  for p in all_press:
                        #print('Merging loop, pressure:: ' , p )                       
                        for v in all_var:                              
                              """ Dataset with no data ( the dataset is not a key of  the dictionary_data[dt] 
                                   This means that there is not data for this dt, for this pressure level, for this dataset 
                                   I assing the dummy value -7777 for this dataset """
                              ...
                              for k in dictionary_data[dt].keys() :
                                    ...
                                    small_df = self.data[k]['dataframe'] [index:index_up]
                                    
                                    
                                    """ Variables to be extracted from the total dataframe """
                                    variables = ['z_coordinate', 'z_coordinate_type' , 'date_time', 'observation_value', 'observed_variable', 'observation_id', 'latitude' , 'longitude' ]
                                    lists = [ small_df[v].values for v in variables ]
                                    
                                    for (indx, z, z_type, dT , obs_val, obs_var, ids, lat, lon ) in zip(small_df.index , * lists) : # unpacking the lists 
                                          """
                                          #It should never happen in principle 
                                          if (dT != dt)== True :  
                                                print('Should never happen')
                                                #sys.exit()
                                          """ 
                                           
                                          if dT == dt and float(z) == float(p) and obs_var == v:
                                    ...
                                    merged_data[k].append(value)
                                    merged_data[k+'_id'].append(Id)
                                    merged_data[k+'_lat'].append(lat)
                                    merged_data[k+'_lon'].append(lon)
                                          
                                    '''
                                    try:
                                          loc = dataframe_data[k].loc[ (dataframe_data[k]['date_time'] == dt) & (dataframe_data[k]['z_coordinate'] == p)  & (dataframe_data[k]['observed_variable'] == v) ]
                                          value = loc['observation_value'].values[0]
                                          ids     = loc['observation_id'].values[0]
                                          index = loc.index.values[0]
                                          dataframe_data[k] = dataframe_data[k].drop([index])
                                    except:
                                          value = np.nan
                                          
                                    '''      
                                    
                              merged_data['date_time'].append(dt) 
                              merged_data['observed_variable'].append(v) 
                              merged_data['z_coordinate'].append(p) 
                        
                                                      
                  #for k in  self.datasets_keys:                        
                  #      starting_indices[k] = [max(starting_indices[k] ) ]  # update the starting index for the next iteration of the dataframe 
                  
                  if len(merged_data['date_time']) == 0: # no datetime available
                        print('No date_time: I continue ')
                        continue
                  
                  
                  summedRecord = pd.DataFrame.from_dict(merged_data) # creating a DF from the summed data                   
                  indices, best_ds , most_records  , all_ds = self.AnalizeDecideMergeRecord(summedRecord, dt) # returns the indices in the original summedRecord, the name of the ds chosen, the number of pressure levels 
                 
                  """ This can happen if there is a NaN as observed value in e.g. the era5 datasets (you have pressure but no data) """
                  if not best_ds:
                        continue 
      
                  MergedRecord = summedRecord.iloc[indices[:] ]  # extracts the rows corresponding to the desired indices i.e. for the chosen best dataset  
                  
                  Merged = MergedRecord.loc[ : , ['date_time' , 'z_coordinate' ,  'observed_variable', best_ds,  best_ds+'_id' , best_ds +  '_lat',  best_ds + '_lon'] ]
                  ...
                  for ids,d in zip( Merged['source_id'], Merged['observation_id' ] ):
                        if d < 99999999:    
                              merged_id.append (self.observation_ids_merged[ids] * 1000000000 + d )
                        else: 
                              print('Must increase the unique id !!!')
                              merged_id.append (self.observation_ids_merged[ids] * 10000000000 + d )
            ...
            print('*** Concatenating the dataframes  ' , datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'))      
            Merg = pd.concat (all_merged_df)
            print('*** Finished concatenating the dataframes  ' , datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'))      
            self.MergedDF = Merg       
