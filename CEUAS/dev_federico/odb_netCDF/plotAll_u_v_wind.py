...
class Plotter():
     ...
     def style_dic(self):          
          dic_prop = { 'uwind':{'xlab': 'Year' , 'ylab':'Component Speed [m/s]','leg':'uwind'      ,'ax':[1920,2019,-100,100], 'c': 'blue'  } ,
                       'vwind':{'xlab': 'Year' , 'ylab':'Component Speed [m/s]','leg':'vwind'      ,'ax':[1920,2019,-100,100], 'c': 'lime'  } ,
                       'temp' :{'xlab': 'Year' , 'ylab':'Temperature [K]'      ,'leg':'Temperature','ax':[1920,2019,150 ,300], 'c': 'orange'  } ,
     ...
     def plotter_prop(self):
          ...
          plt.xlabel(dic_prop[var]['xlab'], fontsize = fnt_size)
          plt.ylabel(dic_prop[var]['ylab'], fontsize = fnt_size)
          plt.legend(loc = 'lower left', fontsize = 12)
          
     def plotter(self,out_dir=''):
          self.load_xy()                     
          x = self.x_data 
          y = self.y_data
