...
class Utils():
    ...
    def distance(self, lat1,lon1,lat2,lon2):
        ...
        if lat1 == -90.00000000000001:
            lat1 = -90.0
        if lat2 == -90.00000000000001:
            lat2 = -90.0
...
class Data():
    ...
    def __init__(self, dataset='', utils = '', inv=''):
        ...
        self.inventory=inv
        """
        self.datasets = { 'era5_1': '/mnt/users/scratch/leo/scratch/era5/odbs/1/new' , 
    ...
    def read_file(self, file):
        ...
        f = 'temp_data_new/' + self.dataset + '/' + self.file.split('/')[-1] + '.csv'
    ...
    def read_era5_1_mobile(self):
        ...
        pd.DataFrame(dic, index=[0]).to_csv( 'temp_data_new/'  + self.dataset +  '/' + self.file.split('/')[-1] + '.csv', sep = '\t')
    ...
    def read_odb(self):
        ...
        if max(distances) > 30:
            ...
            grouped.to_csv( 'temp_data_new/' + self.dataset  + '/' + self.file.split('/')[-1] + '_coordinates.csv', sep = '\t' )        
        ...
        pd.DataFrame(dic).to_csv( 'temp_data_new/' + self.dataset + '/' + self.file.split('/')[-1] + '.csv', sep = '\t' )        
        self.statid = statids
    ...
    def read_bufr_cnr(self):
        ...
        # reading data via csv 
        data =pd.read_csv(self.file, sep='\t')
        dates = pd.to_datetime(data['date'].astype('string') + data['time'].astype('string').replace({'2147483647':'00'}, regex=True).str.zfill(6), format='%Y%m%d%H%M%S')
        dates = np.unique(dates)     
        ...
        statIds = [ bufr_cnr_file.split('/')[-1].split('_')[0].replace('.csv', '') ]
          
        #lats, lons = list(df.lat.values), list(df.lon.values)
        
        a = self.check_consistent_coords(lats, lons)
    
        dic = { 'max_date': max(dates).astype('datetime64[D]'), 
                 'min_date': min(dates).astype('datetime64[D]'), 
                 'statid': str(statIds), # needed in case multiple ids 
                 'lats': [list(df.lat.values)], 
                 'lons': [list(df.lon.values)], 
        ...
        self.lats = lats
        self.lons = lons 
        self.min_date = min(dates).astype('datetime64[D]')
        self.max_date = max(dates).astype('datetime64[D]')
        self.variables = dic['variables']
        return 0    
...
def wrapper(data, file):
    ...
    out_dir_inv = 'inventories_new/' + dataset 
    ...
    if df_red.empty or data.dataset in ['npsound', 'era5_1_mobile' , 'era5_2_mobile' , 'shipsound' ] :
        ...
        a = open('inventories_new/' + dataset +'/logs/' + dataset + '_processed_all.txt', 'a+')
        a.write(name_s + '\t' + flag + '\t' + '' +  '\n')       

        df = pd.DataFrame( all_inventories, index=[0] )  ### case: fully orphans, no whatsoever matching 
        df['WIGOS_best'] = wigos_pre + st
        name = 'inventories_new/' + dataset + '/' + data.file.split('/')[-1] + '_' + flag + '.csv'            
        ...
        if not data.consistent_coord:  # coordinates are inconsistent, skipping 
            ...
            a = open( 'inventories_new/' + dataset + '/logs/' + dataset + '_' + flag + '.txt', 'a+')
                ...
                if not df.empty: 
                    ...
                    if df.distance_km_minusLat.values[0]:
                        ...
                        if d <= 30:
                            ...
                            if not os.path.isfile('inventories_new/' + dataset + '/logs/' + dataset + '_' + flag +'.txt'):
                                w = open('inventories_new/' + dataset + '/logs/' + dataset + '_' + flag + '.txt', 'w' )
    ...
    if data.dataset == 'era5_1759':
        ...
        if not df_red.empty:
            a = open( 'inventories_new/' + dataset + '/logs/' + dataset + '_CHECK_OSCAR_SignLatMismatch.txt', 'a+' )        
    ...
    name = 'inventories_new/' + dataset + '/' + data.file.split('/')[-1] + '_inventories.csv'
    ...
    if len(df_red) >0 and data.consistent_coord:
        ...
        out = open( 'inventories_new/' + dataset + '/logs/' + dataset + '_unidentified.txt', 'a+')
        out.write( name_s +   '\n' )
    ...
    #     a = open('inventories_new/' + dataset + '/logs/' + dataset +"_failed_files.txt", "a+")
    #     a.write(name_s + '\n')       
...
def get_flist(db):
    ...
    if db not in ['era5_1_mobile']:
        flist = [f for f in flist if '.nc' not in f and '.gz' not in f ]
...
if __name__ == '__main__':
    ...
    databases = ['era5_1'] # , 'era5_1_mobile']  
    ...
    for db in databases:
        ...
        if not os.path.isdir( 'inventories_new/' + db + '/logs'):
            os.makedirs( 'inventories_new/' + db + '/logs' )
        if not os.path.isdir( 'temp_data_new/' + db + '/logs'):
                os.makedirs( 'temp_data_new/' + db + '/logs' )            
        ...
        '''
        flist_n = [f.split('_inventories')[0] for f in os.listdir('inventories_new/' + db ) if 'noMatchingCoordNoIds'  in f]
        ...
        if db != 'igra2':   
            if CHECK_MISSING:
                ...
                try: 
                    processed = [f.replace(".csv","") for f in os.listdir('temp_data_new/'+db)] 
                except:
                    processed = []
        ...
        #fii = [f.replace('_coordinates','').replace('.csv','') for f in os.listdir('/users/staff/federico/GitHub/CEUAS_master_SEPTEMBER2021/CEUAS/CEUAS/meta/inventory_comparison/code/temp_data_new/' + db) ]
        #flist = [f for f in flist if f.split('/')[-1]  not in fii ]
        
        if POOL: # running multiprocessing or single 
            ray.init(num_cpus=n_pool)
            ray_data = ray.put(data)
            result_ids = []
            for i in flist[:1]:
                result_ids.append(wrapper.remote(ray_data,i))
