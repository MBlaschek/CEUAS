    ...
    #     df = eua.CDMDataset(filename = statlist[0]).to_dataframe(groups=['observations_table'], variables=['observed_variable', 'observation_value', 'date_time', 'z_coordinate', 'latitude', 'longitude'])
        ...
        #                             print(date)
            ...
            try:
                ...
                pass
            if day:
                pickle.dump( refl, open( "rttov_out/"+statid+"/RISE_"+statid+"_day_refl.p", "wb" ) )
                pickle.dump( dates, open( "rttov_out/"+statid+"/RISE_"+statid+"_day_dates.p", "wb" ) )
                pickle.dump( chan_list, open( "rttov_out/"+statid+"/RISE_"+statid+"_day_chan_list.p", "wb" ) )
                print('done: '+statid)
            else:
