...
def calc_station(statid, chum):
    ...
    #     df = eua.CDMDataset(filename = statlist[0]).to_dataframe(groups=['observations_table'], variables=['observed_variable', 'observation_value', 'date_time', 'z_coordinate', 'latitude', 'longitude'])
        df = eua.CDMDataset(filename = statlist[0]).to_dataframe(groups=['observations_table', 'advanced_homogenisation'], variables=['observed_variable', 'observation_value', 'date_time', 'z_coordinate', 'latitude', 'longitude', 'RAOBCORE_bias_estimate'])
        ###
        ...
        ##
        all_dfta.ta = all_dfta.ta - all_dfta.RAOBCORE_bias_estimate
        ##
        ...
        #                             print(date)
            ...
            try:
                ...
                pass
            if day:
                pickle.dump( refl, open( "rttov_out/"+statid+"/RAOBCORE_"+statid+"_day_refl.p", "wb" ) )
                pickle.dump( dates, open( "rttov_out/"+statid+"/RAOBCORE_"+statid+"_day_dates.p", "wb" ) )
                pickle.dump( chan_list, open( "rttov_out/"+statid+"/RAOBCORE_"+statid+"_day_chan_list.p", "wb" ) )
                print('done: '+statid)
            else:
                pickle.dump( refl, open( "rttov_out/"+statid+"/RAOBCORE_"+statid+"_night_refl.p", "wb" ) )
                pickle.dump( dates, open( "rttov_out/"+statid+"/RAOBCORE_"+statid+"_night_dates.p", "wb" ) )
                pickle.dump( chan_list, open( "rttov_out/"+statid+"/RAOBCORE_"+statid+"_night_chan_list.p", "wb" ) )
                print('done: '+statid)
    except:
        print('error: '+statid)
