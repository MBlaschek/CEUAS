        ...
        #                         for j in [1000,2000,3000,5000,7000,10000,15000,20000,25000,30000,40000,50000,70000,85000,92500,100000]:
                        ...
                        if len(mon_mean) < 9:
                            ...
                            if len(high_mon_mean) < 9:
                                b_out = np.array([[np.nan, np.nan, np.nan]])
                            else:
                                plevs_to_check = high_mon_mean.index
                                if(not (3000 in plevs_to_check and 5000 in plevs_to_check and 7000 in plevs_to_check and 10000 in plevs_to_check and 
                                    ...
                                    high_reduced_sh = reduced_sh[reduced_sh.index.isin([3000,5000,7000,10000,15000,20000,25000,30000,40000,50000,70000])]
                                    a, b = rttov_calc(high_mon_mean,high_reduced_sh, era_input, date)
                                    b_out = np.array([[np.nan, b[0][1], b[0][2]]])
                            ...
                            if len(low_mon_mean) < 9:
                                b_out = np.array([[np.nan, b_out[0][1], b_out[0][2]]])
                            else:
                                ...
                                if(not (5000 in plevs_to_check and 7000 in plevs_to_check and 10000 in plevs_to_check and 
                                    ...
                                    high_reduced_sh = reduced_sh[reduced_sh.index.isin([3000,5000,7000,10000,15000,20000,25000,30000,40000,50000,70000])]
                                    a, b = rttov_calc(high_mon_mean,high_reduced_sh, era_input, date)
                                    b_out = np.array([[b[0][0], b_out[0][1], b_out[0][2]]])
            ...
            try:
                ...
                pass
            if day:
                pickle.dump( refl, open( "rttov_out/"+statid+"/"+statid+"_day_refl.p", "wb" ) )
                pickle.dump( dates, open( "rttov_out/"+statid+"/"+statid+"_day_dates.p", "wb" ) )
