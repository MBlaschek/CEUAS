...
def calc_station(statid, chum, adj = None):
    ...
    try:
        ...
        for day in [True,False]:
            ...
            for yr in range(1979,2022,1):
                for mon in range(int(str(yr)+'01'), int(str(yr)+'13'), 1):
                    ...
                    mon_mean = mon_mean.dropna(subset=['ta'])
                    reduced_sh = chum
        ...
        try:
            os.makedirs("./rttov_out_ih/"+statid+"/")
        except:
            pass

        if adj == None:
            pickle.dump( b_final, open( "rttov_out_ih/"+statid+"/IGRAH_"+statid+"_day_refl.p", "wb" ) )
            pickle.dump( wholemon[:middle_index], open( "rttov_out_ih/"+statid+"/IGRAH_"+statid+"_day_dates.p", "wb" ) )
        ...
        if adj == None:
            pickle.dump( b_final, open( "rttov_out_ih/"+statid+"/IGRAH_"+statid+"_night_refl.p", "wb" ) )
            pickle.dump( wholemon[middle_index:], open( "rttov_out_ih/"+statid+"/IGRAH_"+statid+"_night_dates.p", "wb" ) )
        print('night done: '+statid)
    except:
        print('nothing to calculate: '+statid)
#         if adj == None:
#             pickle.dump( np.array([[[np.nan, np.nan, np.nan]]]), open( "rttov_out_ih/"+statid+"/IGRAH_"+statid+"_day_refl.p", "wb" ) )
#             pickle.dump( [197901], open( "rttov_out_ih/"+statid+"/IGRAH_"+statid+"_day_dates.p", "wb" ) )
#         print('nothing to calculate: '+statid)
        
#         if adj == None:
#             pickle.dump( np.array([[[np.nan, np.nan, np.nan]]]), open( "rttov_out_ih/"+statid+"/IGRAH_"+statid+"_night_refl.p", "wb" ) )
#             pickle.dump( [197901], open( "rttov_out_ih/"+statid+"/IGRAH_"+statid+"_night_dates.p", "wb" ) )
#         print('nothing to calculate: '+statid)
...
if __name__ == '__main__': 
    ...
    df = eua.CDMDataset(filename = statlist[0]).to_dataframe(groups=['observations_table'], variables=['observed_variable', 'observation_value', 'date_time', 
                                                                                                       'z_coordinate', 'latitude', 'longitude',])
...
#     stats = glob.glob('./igra_h_single_stats_new/*11035*')
    ...
    func=partial(calc_station, chum = dfsh)
