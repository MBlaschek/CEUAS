...
###
        if adj == None:
            ...
            df = eua.CDMDataset(filename = statlist[0]).to_dataframe(groups=['observations_table', 'advanced_homogenisation'], variables=['observed_variable', 'observation_value', 'date_time', 'z_coordinate', 'latitude', 'longitude', adj+'_bias_estimate'])
        ###
        df = df[df.z_coordinate.isin([5000,7000,10000,15000,20000,25000,30000,40000,50000,70000,85000])]
        df = df[df.date_time > '1979']
        df = df.rename({'latitude':'lat','longitude':'lon', 'date_time':'time', 'z_coordinate':'plev'}, axis='columns')

        all_dfsh = df[df.observed_variable == 85]
        ...
        for day in [True,False]:
            ...
            for yr in range(1979,2022,1):
                for mon in range(int(str(yr)+'01'), int(str(yr)+'13'), 1):
                    ...
                    reduced_sh = dfsh
                    for j in [5000,7000,10000,15000,20000,25000,30000,40000,50000,70000,85000]:
                    ...
                    if len(mon_mean) >= 9:
                        ...
                        high_mon_mean = mon_mean[mon_mean.index.isin([5000,7000,10000,15000,20000,25000,30000,40000,50000,70000])]
                        if len(high_mon_mean) >= 9:
                            ...
                            if((5000 in plevs_to_check and 7000 in plevs_to_check and 10000 in plevs_to_check and 
                                ...
                                pressdata34.append(high_mon_mean.index)
        ...
        if adj == None:
            ...
            pickle.dump( b_final, open( "rttov_out/"+statid+"/"+adj+"_"+statid+"_day_refl.p", "wb" ) )
            pickle.dump( wholemon[:middle_index], open( "rttov_out/"+statid+"/"+adj+"_"+statid+"_day_dates.p", "wb" ) )
        ...
        if adj == None:
            ...
            pickle.dump( b_final, open( "rttov_out/"+statid+"/"+adj+"_"+statid+"_night_refl.p", "wb" ) )
            pickle.dump( wholemon[middle_index:], open( "rttov_out/"+statid+"/"+adj+"_"+statid+"_night_dates.p", "wb" ) )
...
if __name__ == '__main__': 
    try:
        os.makedirs("./rttov_out/")
    except:
        pass
#     [5000,7000,10000,15000,20000,25000,30000,40000,50000,70000,85000]
    consthum = np.array([5.90, 9.17, 20.30,
