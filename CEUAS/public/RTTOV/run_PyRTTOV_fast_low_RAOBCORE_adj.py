            ...
            for h in [0,12]:
                if h == 0:
                    ...
                    for t in range(len(adjs_date)-1):
                        mask = (adjs_date[t] < df.time) & (df.time < adjs_date[t+1]) & (df.plev == p) & (df.hour == 0)
                ...
                if h == 12:
                    ...
                    adjs_date = np.array(im.datum)
                    for t in range(len(adjs_date)-1):
                        mask = (adjs_date[t] < df.time) & (df.time < adjs_date[t+1]) & (df.plev == p) & (df.hour == 12)
                        df.loc[mask, 'temperatures'] -= adjs_val[t]
...
#             [2]
        ...
        if adj == None:
            ...
            pickle.dump( b_final, open( odir+"/"+statid+"/"+adj+"_"+statid+"_day_refl.p", "wb" ) )
            pickle.dump( wholemon[:middle_index], open( odir+"/"+statid+"/"+adj+"_"+statid+"_day_dates.p", "wb" ) )
