...
###
    ...
    #         df = df[df.z_coordinate.isin([30000,40000,50000,70000,85000])]
    ...
    #                 for mon_tb in ['01','02','03','04','09','10','11','12']:
    #                 mon = int(str(yr)+mon_tb)
                    wholemon.append(mon)
    ...
    #                     for j in [30000,40000,50000,70000,85000]:
    ...
    #         a2,b2 = rttov_calc(np.array(tadata)[np.array(chandata) == 2], np.array(humdata)[np.array(chandata) == 2], np.array(pressdata)[np.array(chandata) == 2], [x for x, y in zip(eradata, chandata) if y == 2], np.array(datedata)[np.array(chandata) == 2], [1,2,3])
