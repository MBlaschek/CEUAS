...
subroutine ksmov(test,ref,ni,istart,istop,maxlen,increment,pcount,mcount,miss_val,max_miss,ks,ksz)


implicit none

integer         :: ni,maxlen,max_miss,istart,istop
real(kind=JPRM) :: miss_val

real(kind=JPRM) :: test(ni),ref(ni),diff(ni)
real(kind=JPRM) :: ks(ni),ksz(ni)

real            :: plus(maxlen/2),minus(maxlen/2),d,z,p,sx(maxlen/2),sy(maxlen/2)
integer         :: pcount(ni),mcount(ni)

integer         :: increment,m2,lm,lp,ifail,i,j
logical         :: index(ni)

if(istart .eq. 1 .and. istop .eq. ni) then
ks=miss_val
else
ks(istart:istop)=miss_val
endif
diff=miss_val
...
do i=1,ni,increment
  if(maxlen/2-pcount(i) .lt. max_miss .and. maxlen/2-mcount(i) .lt. max_miss) then
    lp=0
    lm=0
    do j=1,maxlen/2
      if(index(i-maxlen/2+j)) then
        lp=lp+1
        plus(lp)=diff(i-maxlen/2+j)
      endif
      if(index(i+j)) then
        lm=lm+1
        minus(lm)=diff(i+j)
    ...
    enddo
    if(lm .ne. mcount(i) .or. lp .ne. pcount(i)) then
!      print*,i,lm,mcount(i),lp,pcount(i)
    endif
    CALL G08CDF(lp, plus, lm, minus, 1, D, Z, P, SX, SY, IFAIL)
    ks(i:i+increment-1)=P
    ksz(i:i+increment-1)=Z
  endif
enddo

return
end subroutine ksmov

subroutine ksmoveqsamp(test,ref,ni,istart,istop,maxlen,increment,pcount,mcount,miss_val,max_miss,ks,ksz)

implicit none

integer         :: ni,maxlen,max_miss,istart,istop,jstart,jend,jstartlen,jendlen,k
real(kind=JPRM) :: miss_val

real(kind=JPRM) :: test(ni),ref(ni),diff(ni),bin(ni)
real(kind=JPRM) :: ks(ni),ksz(ni)

real            :: plus(maxlen/2),minus(maxlen/2),d,z,p,sx(maxlen/2),sy(maxlen/2)
integer         :: pcount(ni),mcount(ni),ibin,pmon(12),mmon(12)

integer         :: increment,m2,lm,lp,ifail,i,j,ipc,imc
logical         :: ini,ip(maxlen/2),im(maxlen/2),iph(maxlen/2),imh(maxlen/2)
...
!    im=.false.
  ...
  if(maxlen/2-ipc .lt. max_miss .and. maxlen/2-imc .lt. max_miss) then
    ...
    do i=1,jstartlen
      if(ip(i)) then
        lp=lp+1
        plus(lp)=diff(jstart+i-1)
      endif
    enddo
    do i=1,jendlen
      if(im(i)) then
        lm=lm+1
        minus(lm)=diff(j+m2+i-1)
      endif
    enddo
    if(lm .ne. imc .or. lp .ne. ipc) then
      print*,i,lm,mcount(i),lp,pcount(i)
    endif
    if(j .eq. 31) then
!      write(*,*) j
    endif
!    write(11,'(I6,2F5.2)') j,sum(plus(1:lp))/lp,sum(minus(1:lm))/lm
    CALL G08CDF(ipc, plus,imc, minus, 1, D, Z, P, SX, SY, IFAIL)
    ks(j+m2:j+m2+increment-1)=P
    ksz(j+m2:j+m2+increment-1)=Z
    pcount(j+m2:j+m2+increment-1)=ipc
    mcount(j+m2:j+m2+increment-1)=imc
  endif
enddo

where(diff .eq. miss_val) 
  ks=miss_val
  ksz=miss_val
endwhere

return
end subroutine ksmoveqsamp

subroutine mwmov(test,ref,ni,istart,istop,maxlen,increment,pcount,mcount,miss_val,max_miss,mw)
...
real(kind=JPRM) :: test(ni),ref(ni),diff(ni)
real(kind=JPRM) :: mw(ni)

real            :: plus(maxlen/2),minus(maxlen/2),U,UNOR,P,ranks(maxlen),wrk(maxlen)
integer         :: pcount(ni),mcount(ni)

integer         :: increment,m2,lm,lp,ifail,i,j
logical         :: index(ni),TIES

mw(istart:istop)=miss_val
diff=miss_val
...
!      print*,i,lm,mcount(i),lp,pcount(i)
    ...
    CALL G08AHF(lp, plus, lm, minus, 'T', U, UNOR, P, TIES, RANKS, WRK,IFAIL)
    mw(i:i+increment-1)=UNOR
  endif
enddo

return
end subroutine mwmov
