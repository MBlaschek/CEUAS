...
def homogenize_winddir(via_backend=False, fns=[]):
    ...
    lplot=False
    with open(os.path.expanduser('/home/srvx11/lehre/users/a1400070/scratch/hug/config/active.json')) as f:
        active=json.load(f)
    ...
    for i in range(lats.shape[0],lats.shape[0]):
        ...
        if len(idx)>1:
            ...
            try:
                ...
                with h5py.File('/raid/home/srvx11/lehre/users/a1400070/CEUAS/CEUAS/public/adjust/Wind_adjustment/'+ids[idx[0]]+'_CEUAS_merged_v1.nc','r') as f:
                    with h5py.File('/raid/home/srvx11/lehre/users/a1400070/CEUAS/CEUAS/public/adjust/Wind_adjustment/'+ids[idx[1]]+'_CEUAS_merged_v1.nc','r') as g:
                        try:
...
#     r = http.request('GET', 'http://srvx8.img.univie.ac.at:8000/statlist/?mindate=1900-01-01&enddate=2020-12-31')
#     fns=r.data.split(b'\n')
...
def homogenize_station(opath,via_backend,fnf):
    ...
    if via_backend:
        ...
        try:        
            ...
            ifile=glob.glob('/raid/home/srvx11/lehre/users/a1400070/CEUAS/CEUAS/public/adjust/Wind_adjustment/'+fnf+'_CEUAS_merged_v1.nc')[0]
            #try:
            ...
            data=dict(zip(['d','u','v'],eua.vm_request_wrapper({'variable': ['wind_direction','u_component_of_wind','v_component_of_wind'],  
                                        ...
                                        overwrite=True,vm_url='http://srvx8.img.univie.ac.at:8000').values()))
...
#         try:
        ...
        for k in raggedd.keys():
            ...
            if k=='d':
                ...
                data.write_observed_data('wind_bias_estimate_2',
                ...
                with h5py.File(ifile,'r+') as data:
                    ...
                    data.file['advanced_homogenisation']['wind_bias_estimate_2'][cdict[k]['istart']:cdict[k]['istop']]=raggedd[k]
