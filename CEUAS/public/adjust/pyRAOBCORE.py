...
def do_test(fg_dep,days,RC):
    ...
    tsarad=np.zeros_like(fg_dep,shape=(1,6,fg_dep.shape[2]))
...
def break_analyze(finfo):
    ...
    while np.max(total)>RC['snht_thresh']*10:
...
def RAOB_findbreaks(method,fn):
    ...
    finfo['tsas'],finfo['tsarad']=do_test(finfo['fg_dep'],finfo['days'],RC)
...
def RAOB_adjust(finfo):
    ...
    return finfo['adjustments'],finfo['adjusted_temperatures']
...
if __name__ == '__main__':
    ...
    if False:
        ...
        x=np.empty((20,1000,1460),dtype=np.float32)
        imax,rmax,tsas=break_simulator(x)
        plt.figure(figsize=(20,15))
        fi=[]
        fr=[]
        for im in range(imax.shape[0]):
            plt.subplot(5,4,im+1)
            hi=plt.hist(imax[im,:].flatten(),density=True,bins=20)
            fi.append(scs.laplace.fit(imax[im,:]))
            plt.plot(hi[1]+0.5,scs.laplace.pdf(hi[1],*fi[-1]))
            plt.title(str(im))
        plt.tight_layout()
        plt.show()
        plt.figure(figsize=(20,15))
        for im in range(imax.shape[0]):
            plt.subplot(5,4,im+1)
            hr=plt.hist(rmax[im,:].flatten(),density=True,bins=20)
            fr.append(scs.norm.fit(rmax[im,:]))
            plt.plot(hr[1],scs.norm.pdf(hr[1],*fr[-1]))
            plt.title(str(im))
        plt.tight_layout()
        plt.show()
        plt.subplot(1,2,1)
        plt.plot(np.asarray(fi)[:,1])
        plt.subplot(1,2,2)
        plt.plot(np.asarray(fr)[:,1]/np.asarray(fr)[:,0])
        plt.plot(np.asarray(fr)[:,0])
        plt.show()
    ...
    #breaks=list(map(break_analyze,finfo))
    #for i in range(len(finfo)): 
        #finfo[i]['breaks']=breaks[i]
