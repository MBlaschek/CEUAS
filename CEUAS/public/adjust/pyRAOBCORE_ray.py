...
def RAOB_adjust(finfo):
    ...
    return ray.put({'goodbreaklist':np.array(finfo['goodbreaklist']),'break_profiles':break_profiles[goodbreaklist,:,:],
            ...
            for name, variable in src.variables.items():
                if name == 'datum':
                    ...
                    if name in ['montemp','goodmon']:
                        ...
                        dst[name][:,:RC['pidx'].shape[0],:] = fi['mtemperatures']-fi['madjusted_temperatures']
...
def write_adjustment(ipath,opath,prefix,fi,res_fi,rich_ref0=None,rich_ref1=None,initial_adjust_RAOB=None,initial_adjust_RICH=None):
    
    if len(res_fi['goodbreaklist'])==0:
    ...
    try:
        with netCDF4.Dataset(fni) as src, netCDF4.Dataset(fno, "w") as dst:
            ...
            for name, dimension in src.dimensions.items():
                if name =='time':
                    dst.createDimension(name, (len(res_fi['goodbreaklist'])+2 if not dimension.isunlimited() else None))
                else:
                    dst.createDimension(
                        name, (len(dimension) if not dimension.isunlimited() else None))
            # copy all file data except for the excluded
            for name, variable in src.variables.items():
                if name == 'datum':
                    x = dst.createVariable(name, variable.datatype, variable.dimensions)
                    dst[name][:] = np.concatenate((np.array([1]),fi['days'][res_fi['goodbreaklist'][:]],np.array([44998])),axis=0)
                    # copy variable attributes all at once via dictionary
                    dst[name].setncatts(src[name].__dict__)
                    ...
                    if rich_ref1:
                        ...
                        if 'rio' in prefix and rich_ref1:
                            ...
                            if initial_adjust_RICH:
                                if 'initial_adjustments' in initial_adjust_RICH.keys():
                                    ...
                                    dst[name][:,:RC['pidx'].shape[0],:] = addini(rich_ref1['obs']['rich_adjustments'],np.zeros(res_fi['adjustments'].shape[:2]))
                            else:
                                dst[name][:,:RC['pidx'].shape[0],:] = addini(rich_ref1['obs']['rich_adjustments'],np.zeros(res_fi['adjustments'].shape[:2]))
                            ...
                            if initial_adjust_RICH:
                                if 'initial_adjustments' in initial_adjust_RICH.keys():
                                    ...
                                    dst[name][:,:RC['pidx'].shape[0],:] = addini(rich_ref1['tau']['rich_adjustments'],np.zeros(res_fi['adjustments'].shape[:2]))
                            else:
                                dst[name][:,:RC['pidx'].shape[0],:] = addini(rich_ref1['tau']['rich_adjustments'],np.zeros(res_fi['adjustments'].shape[:2]))
                    else:
                        x = dst.createVariable(name, variable.datatype, variable.dimensions)
                        ...
                        #print(fi['sid'][-6:],initial_adjust_RAOB.keys(),res_fi['adjustments'].shape)
                        if initial_adjust_RAOB:
                            if 'initial_adjustments' in initial_adjust_RAOB.keys():
                                
                                dst[name][:,:RC['pidx'].shape[0],:] = addini(res_fi['adjustments'],initial_adjust_RAOB['initial_adjustments'])
                            else:
                                dst[name][:,:RC['pidx'].shape[0],:] = addini(res_fi['adjustments'],np.zeros(res_fi['adjustments'].shape[:2]))
                        else:
                            dst[name][:,:RC['pidx'].shape[0],:] = addini(res_fi['adjustments'],np.zeros(res_fi['adjustments'].shape[:2]))
...
def save_monmean(tfile,l,fi,res_fi,rich_ref0=None,rich_ref1=None,initial_adjust_RAOB=None,initial_adjust_rich=None):
    
    #return ray.put({'break_profiles':break_profiles,'adjusted_temperatures':finfo['adjusted_temperatures']})
    #fnames= fi['sid']
    if type(fi[l]) is not dict:
        fi=ray.get(fi)[l]
        res_fi=ray.get(res_fi)[l]
    else:
        fi=fi[l]
        res_fi=res_fi[l]
    fi['adjustments']=-res_fi['break_profiles']
    fi['adjusted_temperatures']=res_fi['adjusted_temperatures'].copy()
    ...
    plist=['temperatures','adjusted_temperatures','fg']
    if rich_ref0:
    ...
    if initial_adjust_RAOB:
        ...
        if 'initial_adjustments' in initial_adjust_RAOB.keys():
            ...
            for ih in range(sh[0]):
                for ip in range(sh[1]):
                    fi['adjusted_temperatures'][ih,ip,:]-=initial_adjust_RAOB['initial_adjustments'][ih,ip]
    if initial_adjust_rich:
    ...
    write_adjustment(ipath,exper.join(ipath.split('exp03')),prefix,fi,res_fi,
...
def rich_calcadjprofile(ldiff,sdiff,ldays,sdays,tbin,sbin,llmaxlen,slmaxlen):
    #from anomaly import ndnanmean2,nnanmean
    #tsa=np.zeros(ldiff.shape[2],dtype=ldiff.dtype)
    #count=np.zeros((tsa.shape[0],12),dtype=np.int32)
    #tmean=np.zeros((tsa.shape[0],12),dtype=ldiff.dtype)
    #tsquare=np.zeros((tsa.shape[0],12),dtype=ldiff.dtype)
    #lprof=np.zeros(ldiff.shape[:2],dtype=ldiff.dtype)
    #lprofnaive=np.zeros(ldiff.shape[:2],dtype=ldiff.dtype)
    ##for ih in range(ldiff.shape[0]):
        ##for ip in range(ldiff.shape[1]):
            ##lprof[ih,ip]=SNHT.numba_meaneqmov(ldiff[ih,ip,:], tbin,
                      ##tsa, RC['mean_maxlen'],RC['snht_maxmiss'], RC['miss_val'],
                      ##count, tmean, tsquare,kref=llmaxlen)
    ## compare only profiles available in both series - this is important particularly for RICH-obs
    
    #lprof=-ndnanmean2(ldiff[:,:,:llmaxlen],thresh=RC['mean_maxlen']//4-RC['snht_maxmiss'])+ndnanmean2(ldiff[:,:,llmaxlen:],thresh=RC['mean_maxlen']//4-RC['snht_maxmiss'])
    ##def numba_meaneqmov(t, dbin,tsa, n,max_miss, miss_val, count, tmean, tsquare,kref=0):
    #tsa=np.zeros(sdiff.shape[2],dtype=ldiff.dtype)
    #count=np.zeros((tsa.shape[0],12),dtype=np.int32)
    #tmean=np.zeros((tsa.shape[0],12),dtype=ldiff.dtype)
    #tsquare=np.zeros((tsa.shape[0],12),dtype=ldiff.dtype)
    ...
    #sprofnaive=np.zeros(sdiff.shape[:2],dtype=ldiff.dtype)
    ##for ih in range(sdiff.shape[0]):
        ##for ip in range(sdiff.shape[1]):
            ##sprof[ih,ip]=SNHT.numba_meaneqmov(sdiff[ih,ip,:], sbin,
                      ##tsa, RC['mean_maxlen'],RC['snht_maxmiss'], RC['miss_val'],
                      ##count, tmean, tsquare,kref=slmaxlen)
    #sprof=-ndnanmean2(sdiff[:,:,:slmaxlen],thresh=RC['mean_maxlen']//4-RC['snht_maxmiss'])+ndnanmean2(sdiff[:,:,slmaxlen:],thresh=RC['mean_maxlen']//4-RC['snht_maxmiss'])

    ##return sprofnaive-lprofnaive
    #lprof-=sprof
...
#@ifray(ray.remote,RC['richfuture'])
def rich_adjust(l,obj_ref,obj_refRAOB,obj_sdist,rich_iter,obj_rich_ref=None):
    sys.path.append(os.path.expanduser('~/python/Rasotools/rasotools/'))
    sys.path.append(os.path.expanduser('~/python/Rasotools/'))
    import warnings
    ...
    if type(obj_ref) is list: # if called by ray, the first dereferencing is done automatically
        ...
        #ref=ray.get(obj_ref)
        refRAOB=[None for i in range(len(obj_ref))]
        refRAOB[l]=ray.get(obj_refRAOB[l])
        #refRAOB=ray.get(obj_ref)
        sdist=obj_sdist
        if rich_iter!=0:
        ...
        refRAOB=ray.get(obj_refRAOB)
        refRAOB=ray.get(refRAOB) # second dereference - gets actual dictionaries
        
        sdist=ray.get(obj_sdist)
        if rich_iter!=0:
            rich_ref=ray.get(obj_rich_ref)
            rich_ref=ray.get(rich_ref)
    #print('rich_adjust:',l,type(ref),len(ref),type(refRAOB),type(sdist))
    sdidx=np.argsort(sdist[l])

    bll=len(refRAOB[l]['goodbreaklist'])
    if bll>0:       
        if refRAOB[l]['goodbreaklist'][-1]==44998: # initial adjustment was applied if this is the case
            bll=len(refRAOB[l]['goodbreaklist'])-1
                    ...
                    for ri_min_sampsize in RC['ri_min_sampsize']:
                        ...
                        richens.append(dict(params=dict(daynight_method=daynight_method,weight_distance=weight_distance,eqsamp=eqsamp,min_neighbours=min_neighbours,ri_min_sampsize=ri_min_sampsize),
                                            found=dict(break_dates=['' for i in range(bll)],
                                             ...
                                             weight=np.exp(-0.01-sdist[l][sdidx]*6370./weight_distance),
    ...
    for rich in [richens[richcount+1]]:
        ...
        for ib in range(bll-1,-1,-1): # test station breaks       
            tib=ref[l]['days'][refRAOB[l]['goodbreaklist'][ib]] # we need days since 1900 here! breaklist values are indices of time series, not days since 1900
            s=0
            refcount=0
            lblen=0
            si=0
            for si in sdidx[1:]: # try ref stations beginning with nearest:
                ...
                refcount +=1
                if sdist[l][si]*6370. > weight_distance and refcount>500: # second condition is for test stations at remote places
                    print('not enough buddies found','{:5.1f}'.format(sdist[l][si]*6370.),refcount,lblen)
                    break
                if ref[si] is None:
                    ref[si]=ray.get(obj_ref[si])
                    refRAOB[si]=ray.get(obj_refRAOB[si])
                    if rich_iter!=0:
                ...
                #print(ref[si]['sid'],'breaklistref',[0],refRAOB[si]['goodbreaklist'],[sni-1])
                if len(refRAOB[si]['goodbreaklist'])>0:
                    
                    breaklistref=ref[si]['days'][np.concatenate(([0],refRAOB[si]['goodbreaklist'],[sni-1]))] # days since 1900 of breaks of reference station. First and last values added for convenience
                else:
                ...
                if rbm<breaklistref.shape[0]-1:
                    rmaxlen=breaklistref[rbm+1]-tib if rich_iter==0 else np.min([breaklistref[-1]-tib,RC['mean_maxlen']])
                else:
                    rmaxlen=np.min([breaklistref[-1]-tib,RC['mean_maxlen']])
                if rmaxlen<ri_min_sampsize:
                    continue
                if rbm<breaklistref.shape[0]:
                    
                    if ib>0:
                        sibm1=np.searchsorted(ref[si]['days'],refRAOB[l]['goodbreaklist'][ib-1])
                        tibm1=ref[l]['days'][refRAOB[l]['goodbreaklist'][ib-1]] # we need days since 1900 here! breaklist values are indices of time series, not days since 1900
                ...
                if rich_iter==0:
                    ...
                    if not RC['richfuture']:
                        ...
                        comp={'tau':{'test':richtemp['tau']['rich_adjusted_fg_dep'],
                                     'ref':make_adjusted_series(ref[si]['fg_dep'],refRAOB[si]['adjustments'],refRAOB[si]['goodbreaklist'])},
                                'obs':{'test':richtemp['obs']['rich_adjusted_anomalies'],
                                        'ref':make_adjusted_series(ref[si]['anomalies'],refRAOB[si]['adjustments'],refRAOB[si]['goodbreaklist'])}}
                    else:
                        ...
                        try:
                            ...
                            comp={'tau':{'test':richtemp['tau']['rich_adjusted_fg_dep'],
                                         'ref':make_adjusted_series(ref[si]['fg_dep'],rich_ref[si][richcount]['tau']['rich_adjustments'],refRAOB[si]['goodbreaklist'])},
                                       'obs':{'test':richtemp['obs']['rich_adjusted_anomalies'],
                                              'ref':make_adjusted_series(ref[si]['anomalies'],rich_ref[si][richcount]['obs']['rich_adjustments'],refRAOB[si]['goodbreaklist'])}}
                        except KeyError:
                ...
                for method in ['tau','obs']:
                    ...
                    if method=='tau':   
                        ...
                        rich['found']['refsid'][ib].append('{:5.0f} km'.format(sdists[l][si]*6370.))
                    ...
                    for method in 'tau','obs':
                        try:
                            if len(rich[method]['buddies'][ib])>0:                               
                                rich[method]['break_profiles'][ib,:,:]=calc_breakprofiles(np.array(rich[method]['buddies'][ib]),
                                                                                      rich['found']['weight'][:lblen])
                            else:
                        ...
                        if False and ib==15 and method=='obs':
                            ...
                            print('ib')
                        add_breakprofile(refRAOB,l,rich,richtemp,method,ib)
                        #bstart= 0 if ib==0 else refRAOB[l]['goodbreaklist'][ib-1]
                        #for ih in range(bm[1]):
                            #for ip in range(bm[2]):
                                ##comp[method]['test'][ih,ip,bstart:refRAOB[l]['goodbreaklist'][ib]]-=rich[method]['rich_adjustments'][ih,ip,ib]
                                #if method=='tau':  
                                    #richtemp[method]['rich_adjusted_fg_dep'][ih,ip,bstart:refRAOB[l]['goodbreaklist'][ib]]+=rich[method]['rich_adjustments'][ih,ip,ib]
                                #else:
                                    #richtemp[method]['rich_adjusted_anomalies'][ih,ip,bstart:refRAOB[l]['goodbreaklist'][ib]]+=rich[method]['rich_adjustments'][ih,ip,ib]
            ...
            rich['found']['dists'][ib]='{:5.0f} km'.format(sdists[l][si]*6370.)
            if lblen<min_neighbours[rich_iter]:
                print(ref[l]['sid'],tib,'ensemble too small, try supplying RAOBCORE estimate',lblen)
                try:
                    for method in 'tau','obs':
                        #print(rich[method].keys(),refRAOB[l].keys())
                        rich[method]['break_profiles'][ib,:,:]=refRAOB[l]['break_profiles'][ib,:,:]
        ...
        print(richcount,l,len(ref),ref[l]['sid'],[(rich['found']['break_dates'][i],len(rich['tau']['buddies'][i]),
...
def breakplot(l,obj_ref,results_ref,sdist_ref,obj_rich_ref0=None,obj_rich_ref1=None):
    ...
    nbreaks=len(results[l]['goodbreaklist'])
    col=['b','r']
    ...
    for k in rd.keys():
        for method in ['obs','tau']:
            ...
            for ib in range(nbreaks):
                tib=ref[l]['days'][results[l]['goodbreaklist'][ib]]
                plt.subplot(nbreaks//3+1,3,ib+1)
                #plt.subplot(1,1,1)
                xlim=np.nanmin(results[l]['break_profiles'][ib,:,:])-1,np.nanmax(results[l]['break_profiles'][ib,:,:]+1)
                for ih in 0,1:
                    ...
                    plt.semilogy(results[l]['break_profiles'][ib,ih,:],RC['stdplevs'][RC['pidx']],label='RAOBCORE',color=col[ih],linewidth=3,alpha=0.6)
...
def initial_adjust(l,obj_ref,obj_refRAOB,obj_sdist,obj_rich_ref=None,initial_composite_ref=None,rich_initial_composite_ref=None):
    sys.path.append(os.path.expanduser('~/python/Rasotools/rasotools/'))
    sys.path.append(os.path.expanduser('~/python/Rasotools/'))
    ...
    if type(obj_ref) is list: # if called by ray, the first dereferencing is done automatically
        ...
        if initial_composite_ref:
            ...
            initial_composite=None
        if rich_initial_composite_ref:
            rich_initial_composite=ray.get(rich_initial_composite_ref)
        else:
            rich_initial_composite=None
    ...
    if RC['initial_adjustments']=='era5bc' and int(ref[l]['days'][-1]/365.25)>116:
        ...
        if ref[l]['days'].shape[0]-start>RC['snht_min_sampsize']:
            ...
            ini=np.zeros(refRAOB[l]['adjusted_fg_dep'].shape[:2])
            RAOBini['initial_adjustments']= ndnanmean2(refRAOB[l]['adjusted_fg_dep'][:,:,start:],ini,RC['snht_min_sampsize'])
            RAOBini['initial_adjustments'][np.isnan(RAOBini['initial_adjustments'])]=0.
        ...
        if ref[l]['lastsonde']: # and RC['initial_adjustments'] not in ['era5','era5bc']:
            ...
            if any(igood>RC['snht_min_sampsize']):
                ...
                for si in sdidx[1:]: # try ref stations beginning with nearest:
                    ...
                    refcount +=1
                    if sdist[l][si]*6370. > RC['weight_distance'][0] and refcount>500: # second condition is for test stations at remote places
                        print('not enough buddies found','{:5.1f}'.format(sdist[l][si]*6370.),refcount)
                        break
                    ...
                    try:
                        ...
                        if ref[si]['days'].shape[0]-sistart>RC['snht_min_sampsize']:
                            ini=np.zeros(refRAOB[si]['adjusted_fg_dep'].shape[:2])
                            #spagest=ndnanmean2(refRAOB[si]['adjusted_fg_dep'][:,:,sistart:],ini,RC['snht_min_sampsize'])
                            if initial_composite:
                                if 'initial_adjustments' in initial_composite[si].keys():
                                    spagest=ndnanmean2(refRAOB[si]['adjusted_fg_dep'][:,:,sistart:],ini,RC['snht_min_sampsize'])
                                    for ih in range(spagest.shape[0]):
                                        for ip in range(spagest.shape[1]):
                                            ia=initial_composite[si]['initial_adjustments'][ih,ip]
                                            if ia==ia:
                                                spagest[ih,ip]-=ia
                ...
                if len(spagini)>RC['min_neighbours'][0][1]:
                    ini=np.zeros(refRAOB[l]['adjusted_fg_dep'].shape[:2])
                    RAOBini['initial_adjustments']= ndnanmean2(refRAOB[l]['adjusted_fg_dep'][:,:,start:],ini,RC['snht_min_sampsize'])-spaginimean
                    RAOBini['initial_adjustments'][np.isnan(RAOBini['initial_adjustments'])]=0.
                else:
                ...
                if obj_rich_ref:
                                    ...
                                    for method in 'tau','obs':
                                        ...
                                        if rich_initial_composite:
                                            if rich_initial_composite[si]:
                                                if 'initial_adjustments' in rich_initial_composite[si][1][iens][method].keys():
                                                    ...
                                                    for ih in range(rspagest.shape[0]):
                                                        for ip in range(rspagest.shape[1]):
                                                            ia=rich_initial_composite[si][1][iens][method]['initial_adjustments'][ih,ip]
                                                            if ia==ia:
                                                                rspagest[ih,ip]-=ia
                                                    try:
                    ...
                    if len(richensini[24]['obs']['inibuddies'])>RC['min_neighbours'][0][1]:
                        ...
                        for iens in range(len(richensini)):
                            for method in 'tau','obs':
                                ini=np.zeros(refRAOB[l]['adjusted_fg_dep'].shape[:2])
                                richensini[iens][method]['initial_adjustments']=ndnanmean2(refRAOB[l]['adjusted_fg_dep'][:,:,start:],ini,RC['snht_min_sampsize'])
                                richensini[iens][method]['initial_adjustments'][np.isnan(RAOBini['initial_adjustments'])]=0.
...
if __name__ == '__main__':
    ...
    try:
        with open(pkl+'.pkl','rb') as f:
            finfo,results,sdists=pickle.load(f)
    except:
        ...
        #for l in ray.get(obj_ref):
            #breaks[np.array(l['days'][l['breaklist']])]+=1
            #if len(l['breaklist'])>30:
                #print(l['sid'],len(l['breaklist']))
            
        #plt.plot(np.arange(45000)/365.25,breaks)
        #plt.show()
        ...
        if not RC['findfuture']:  
            ...
            for l in range(len(obj_ref)):
                futures.append(ray_RAOB_adjustbreaks.remote('SNHT',obj_ref[l]))
            results_ref = ray.get(futures)
        
        break_sum=np.sum([len(l['goodbreaklist']) for l in ray.get(results_ref)])
        print('Total number of "good" breaks',break_sum)
        #finfo=ray.get(obj_ref)
        #with open('RAOB.pkl','wb') as f:
            #pickle.dump((finfo,ray.get(results_ref),sdists),f)

    #if 'obj_ref' not in locals():
        #obj_ref=[ray.put(f) for f in finfo[:] ]
        #results_ref=[ray.put(r) for r in results[:] ]
        #sdist_ref=[ray.put(s) for s in sdists[:] ]
...
# for some values of RC['initial_adjust'] even the "good" sondes are initial-adjusted
    ...
    for l in lgood: #range(len(obj_ref)):
        futures.append(ray_initial_adjust.remote(l,single_obj_ref,single_results_ref,sdist_ref))
...
# next those with records up to recent times but susptected biases are initial-adjusted
    ...
    for l in lrecent: #range(len(obj_ref)):
        futures.append(ray_initial_adjust.remote(l,single_obj_ref,single_results_ref,sdist_ref,initial_composite_ref=initial_composite_ref))
    iadjustlist = ray.get(futures)
...
# finally those records that end early (before 2010) are initial-adjusted
    ...
    save_monmean(ifile,0,[obj_ref[lx]],[results_ref[lx]],initial_adjust_RAOB=[initial_adjust_ref[lx]])
    print('before write',time.time()-tt)
    futures=[]
    for l in range(len(obj_ref)):
        futures.append(ray_save_monmean.remote(ifile,0,[obj_ref[l]], [results_ref[l]],initial_adjust_RAOB=[initial_adjust_ref[l]]))
    x = ray.get(futures)
    ...
    futures=[]
    for l in range(len(obj_ref)):
        futures.append(ray_rich_adjust.remote(l,single_obj_ref,single_results_ref,sdist_ref,0))
    ...
    single_rich_ref0=ray.put(rich_ref0) 
        ...
        #RC['richfuture']=True
    if False:
        rich_ref0=rich_adjust(lx,single_obj_ref,single_results_ref,sdist_ref,0)
        breakplot(lx,obj_ref,results_ref,sdist_ref,obj_rich_ref0=[rich_ref0])
        single_rich_ref0=ray.put([rich_ref0])
    elif True:
        rich_ref1=rich_adjust(lx,single_obj_ref,single_results_ref,sdist_ref,1,obj_rich_ref=single_rich_ref0)  
        breakplot(lx,obj_ref,results_ref,sdist_ref,obj_rich_ref0=[rich_ref0[lx]],obj_rich_ref1=[rich_ref1])
        save_monmean(ifile,0,[obj_ref[lx]],[results_ref[lx]],rich_ref0=[rich_ref0[lx]],rich_ref1=[rich_ref1])

    print('before second RICH iteration',time.time()-tt)
    futures=[]
    for l in range(len(obj_ref)):
        futures.append(ray_rich_adjust.remote(l,single_obj_ref,single_results_ref,sdist_ref,1,obj_rich_ref=single_rich_ref0))
...
# initial adjustment of RICH composite        
    ...
    futures=[]
    for l in lgood: #range(len(obj_ref)):
        futures.append(ray_initial_adjust.remote(l,single_obj_ref,single_results_ref,sdist_ref,obj_rich_ref=single_rich_ref1))
    ...
    futures=[]
    for l in lrecent: #range(len(obj_ref)):
        futures.append(ray_initial_adjust.remote(l,single_obj_ref,single_results_ref,sdist_ref,obj_rich_ref=single_rich_ref1,initial_composite_ref=initial_composite_ref,
                                        rich_initial_composite_ref=rich_initial_composite_ref))
    irichadjustlist = ray.get(futures)
    for i in range(len(lrecent)): 
...
# this object list is used for final writing.
    ...
    save_monmean(ifile,0,[obj_ref[lx]],[results_ref[lx]],rich_ref1=[rich_ref1[lx]],
    ...
    for l in range(len(obj_ref)):
        ...
        futures.append(ray_save_monmean.remote(ifile,0,[obj_ref[l]], [results_ref[l]],rich_ref1=[rich_ref1[l]],
