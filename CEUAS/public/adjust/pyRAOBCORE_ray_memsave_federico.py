...
#    RC['ri_min_sampsize']=[330,30]
    ...
    RC['richfuture']=True 
    RC['findfuture']=True # set to FALSE to deactivate ray multiproc.
    RC['findfuture']=False # set to FALSE to deactivate ray multiproc.
...
out_dir = '/scratch/das/federico/SNHT_METADATA'
if 'out_dir' not in RC.keys():
    RC['out_dir'] = out_dir 
if 'base_dir' not in RC.keys():
    RC['base_dir'] = os.getcwd()
if RC['plot']:
    if not os.path.isdir(RC['out_dir'] + '/plots/'):
        os.mkdir( RC['out_dir'] + '/plots/' )
...
def break_analyze(finfo):
    ...
    if RC['plot']:
        ...
        for l in range(0,lmax,2):    
            ...
            plt.plot( finfo['days'][::5]/365.25, totalo[l][::5], label='l=' + str(int(l/2)) )
    ...
    plt.savefig( RC['out_dir'] + '/plots/' + finfo['sid']+'_break_analyze.png')   
    plt.close()
    ...
    if RC['plot']:
        ...
        plt.savefig(RC['out_dir'] + '/plots/' + finfo['sid']+'_breakanalysis.png')
        plt.close()
    if RC['savetsa']:
        with open(RC['out_dir'] + '/' + finfo['wigos']+'_breakanalysis.json','w') as f:
...
def RAOB_adjustbs(finfo):
    ...
    if RC['plot']:
        ...
        plt.title(finfo['sid'][-6:])
        plt.tight_layout()
        plt.savefig(RC['out_dir'] + '/plots/' + finfo['sid']+'_adjustments.png')
        #plt.show()
        plt.close()
...
def write_adjustment(ipath,opath,prefix,fi,rich_ref0=None,rich_ref1=None,initial_adjust_RAOB=None,initial_adjust_RICH=None):
    ...
    """ Copy file to new output directory and modify only the copy """
    fno_copy =  RC['out_dir'] + '/' + fno.split('/')[-1]
    #shutil.copy(fno, fno_copy )
    try:
        with netCDF4.Dataset(fni) as src, netCDF4.Dataset(fno_copy, "w") as dst:
            # copy global attributes all at once via dictionary
            new_globals=src.__dict__
            new_globals['history']=datetime.datetime.today().strftime("%m/%d/%Y")
            new_globals['source']='ERA5, IGRA2, NCAR UADB'
...
def save_monmean(tfile,l,exper='exp02',fi={},rich_ref0=None,rich_ref1=None,initial_adjust_RAOB=None,initial_adjust_rich=None):
    ...
    out_dir = RC['out_dir']
    ### write_monmean(ipath,opath,prefix,fi)                                                                                                 

    prefix='feedbackglobbincorrmon'
    write_monmean(ipath,  out_dir ,prefix,fi)

    prefix='feedbackglobbgmon'
    write_bgmonmean(ipath,  out_dir, prefix,fi)

    prefix='feedbackglobbincorrsave'
    write_adjustment(ipath, out_dir , prefix,fi,
...
add e.g. 010393 
i.e. WMO + a 0 in front 

Dont know how to run a weird WIGOS station 
"""