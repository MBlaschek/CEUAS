...
def toyearmonth(itime,units):
    ...
    if 'days since' in units:
        ...
        otime=itime
        for i in range(itime.shape[0]):
            otime[i]=197900+((itime[i]-1)/12)*100+numpy.mod(itime[i]-1,12)+1
    else:
...
def calcdays(startdate,mmax):
    ...
    days=numpy.zeros(mmax,dtype=numpy.int64)
...
#    print 'statcore: ',time.time()-t
    ...
    sig=math.sqrt(numpy.abs(sq-s*s))
    rms=math.sqrt(numpy.abs(sq))
    if short is not None:
        if mima is not None and feld.size>0:
            smima='/%4.1f/%4.1f'%(numpy.nanmin(feld.flatten()),numpy.nanmax(feld.flatten()))
        else:
            smima=''
        ...
        if mima is not None:
            if sig>10:
                smima=', Min: %3.0f, Max: %3.0f'%(numpy.nanmin(feld.flatten()),numpy.nanmax(feld.flatten()))
            else:
                smima=', Min: %4.1f, Max: %4.1f'%(numpy.nanmin(feld.flatten()),numpy.nanmax(feld.flatten()))
            ...
            for ilat in range(hadens.shape[3]):
                for ilon in range(hadens.shape[4]):
                    ...
                    if n>0:
                        ...
                        hadens[ens,ip,index,ilat,ilon]=numpy.nan
            ...
            for ilat in range(hadens.shape[3]):
                for ilon in range(hadens.shape[4]):
                    ...
                    if n>0:
                        ...
                        hadens[ens,ip,l*12+itime,ilat,ilon]=numpy.nan
...
def find_gstatindex(glons,glats,lons,lats,gstatindex):
    ...
    for l in range(lats.shape[0]):
        ilon=int(numpy.floor(lons[l]/(360./glons.shape[0])))
        ilat=int(numpy.floor((lats[l]+90.)/(180./glats.shape[0])))
...
#   currentdata[:]=currentdatatm+tasks[key]["data"][:,iens,:,:,:].reshape(tmshape[0],tmshape[2],ps.shape[0],tmshape[4])[:,:,pindex,:]
    ...
    pindex=numpy.zeros_like(opindex)
...
#   from scipy.stats.stats import nanmean
#   copy strides from up to 8 dimensional arrays
    pindex=numpy.zeros(opindex.shape[0],dtype=numpy.int64)
    pindex[:]=opindex[:]
    pm=int(pindex[0])
    if b.shape[3]==1:
        pindex[0]=0
...
#   copy strides from up to 8 dimensional arrays
    ...
    print((numpy.isnan(dists[0]),dists[0]))
    if dists[0]==0: #nan?
...
def sdistp(dists,x,y,z):
    ...
    idx=numpy.zeros(x.shape[0],dtype=numpy.int32)
...
#   calculate trend heterogeneity cost function
    ...
    tcost=0.
    cost[:]=0
    costk=numpy.zeros_like(cost)
    id=numpy.zeros(slopes.shape[0],numpy.int32)
    s=numpy.zeros_like(dists)
        ...
        if slopes[l]==slopes[l]:
            for k in range(l,slopes.shape[0]):
                if slopes[k]==slopes[k]:     
                    ...
                    s[m]*=s[m]
                    costk[k]+=s[m]
            cost[l]=numpy.nansum(s[id[l]:m+1])
            
    for l in range(slopes.shape[0]):
        cost[l]+=costk[l]
    tcost=numpy.nansum(cost)
    if goodstats>0:
        tcost/=goodstats
        for l in range(slopes.shape[0]):
            cost[l]/=goodstats
...
def thin2(t,n):
    ...
    ni=t.shape[0]
    index=numpy.zeros(ni//n,dtype=int32)-1
    if n<2:
        for i in range(t.shape[0]):
            index[i]=i
    else:
...
def monmean(var,tidx,thresh=15):
    out=numpy.empty((2,tidx.shape[0]))
...
def belttrends(zslopes,belts):
    ...
    weights=numpy.empty(zslopes.shape[0])
    ...
    for k in range(belts.shape[0]):
        ...
        for ip in range(zslopes.shape[1]):
            ...
            b=sum(mask)
            if(b>0):
                beltslopes[k,ip]=sum(hilf[mask]*wk[mask])/numpy.sum(wk[mask])
            else:
