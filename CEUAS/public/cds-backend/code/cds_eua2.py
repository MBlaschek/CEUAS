            ...
            try:
                ...
                with h5py.File(rfile,'r') as f:
                    ...
                    try:
                        ...
                        dsec[-1]+=86399
                        didx=numpy.where(numpy.logical_and(di[0,:]>=dsec[0]-86400,di[0,:]<=dsec[-1]))[0] # -86400 in order to get late launches of previous day
                        if didx.shape[0]==0:
                                ...
                                if k in ['observations_table']: # only obs, feedback fitting criteria (idx) is copied
                                    ...
                                    if 'fbstats' in rvdk:
                                        try:
                                            ...
                                            return '','no '+ccf[rvars['fbstats'][0]]['cdmname']+' found in '+rfile
