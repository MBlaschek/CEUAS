...
#                         try:
                        ...
                        if v in ['observation_value','obsvalue@body']:
                            hilf[mask]=np.nan
                        #hilf = hilf[idx[0]:id(x[-1] + 1]  # use a min:max range
...
def unstack_cube_by_hour(data: xr.DataArray, dim: str = 'time', hour: str = 'hour') -> xr.DataArray:
    ...
    keys = list(data.keys())
    for ikey in keys: #data.keys():
        if np.any(~np.isnan(data[ikey]['relative_humidity'].values)):
...
def to_standard_launch_time(dates: np.datetime64, std_times: list, span: int =3, debug: bool = False):
    ...
    tcorr = np.zeros(hour.size, dtype=int)
...
#                 trange = slice(int(recordindex[itx][0]), int(recordindex[itx][-1]))  # all
    ...
    def read_data_to_3dcube(self, variables: list, dates: list = None, plevs: list = None, feedback: list = None,
        ...
        if self.hasgroups:
            ...
            for ivarnum in varnum:
                ...
                ivarnum.update(kwargs)
                # TIME SLICE, INDEX, SECONDS ARRAY, PRESSURE LEVELS
                iobs, secarray, pressure = self.read_observed_variable(dates=dates,
                                                                                  plevs=plevs,
                                                                                  return_coordinates=True,
                                                                                  **ivarnum)
                ...
                # Convert to Xarray [time x plev x 2]
                ...
                # Convert to Xarray [time x plev]
                if ivarnum['variable'] == 'observation_value':
                    ivar = ivarnum['bkp_var']
                else:
                    ivar = ivarnum['variable']                    
            ...
            data['lat']=xr.DataArray(np.asarray([self.observations_table.latitude[-1]]),name='lat',dims=('station'))
            data['lon']=xr.DataArray(np.asarray([self.observations_table.longitude[-1]]),name='lon',dims=('station'))
                #if ivarnum['variable'] == 'observation_value':
                    ...
                    #data[ivarnum['bkp_var']] = xr.DataArray(iobs,
                                                            ...
                                                            #)
                #else:
                    #data[ivarnum['variable']] = xr.DataArray(iobs,
                                                            #coords=(seconds_to_datetime(secarray[itime]), plevs),
                                                            #dims=('time', 'plev'),
                                                            #name=ivarnum['variable'],
                                                            #attrs=v_attrs,
                                                            #)
    ...
    def write_observed_data(self, name: str, varnum: int,
        ...
        if interpolate and not np.issubdtype(values.dtype, str):
            ...
            tt = time.time()
            @njit(cache=True)
            def fastlint(fvalues, ftime, fplev):
                utime = np.unique(ftime)
                uidx = np.searchsorted(ftime, utime)
                logp = np.log(fplev)
                for m in range(len(uidx) - 1):
                    #islice = slice(uidx[m], uidx[m+1])
                    idx = np.zeros(uidx[m+1]-uidx[m], dtype=np.int32)
                    l = 0
                    for i in range(uidx[m], uidx[m+1]):
                        if fplev[i] == fplev[i] and fvalues[i] == fvalues[i]:
                            idx[l] = i
                            l += 1
                    if l > 1:
                        for ll in range(l-1):
                            if idx[ll+1] - idx[ll] > 1:  
                                for ii in range(idx[ll], idx[ll+1]):
                                    w0 = (logp[ii] - logp[idx[ll]]) / (logp[idx[ll+1]] - logp[idx[ll]])
                                    fvalues[ii] = w0 * fvalues[idx[ll+1]] + (1. - w0) * fvalues[idx[ll]]
                return fvalues

            values = fastlint(farray.values.copy(), farray.time.values, farray.plev.values)

            print(time.time() - tt)
            #fa3 = farray.groupby(data_time).apply(level_interpolation, dim=data_plevs, method='log-linear',
                                                     #extrapolate=extrapolate_plevs, shortcut=False)
            #print(time.time() - tt)
            ...
            match_index = np.arange(values.shape[0], dtype=int)
    ...
    def write_variable(self, name: str,
        ...
        values = data.values
        if interpolate and not np.issubdtype(values.dtype, str):
            mask = np.arange(trange.start, trange.stop, dtype=int)
