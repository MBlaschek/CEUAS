...
def read_hadCRUT4(path,prefix,startyear=1957,endyear=2013,ens=0):
    ...
    try:
        if 'T.4' in prefix:
            ...
            f=numpy.load('CRUT5_'+str(startyear)+'_'+str(endyear)+'.npz')
        ...
        try:
            ...
            hadtem=numpy.zeros_like(hadmed)
            print((fn+' not found'))
...
#	had_rebin_pythran_3672_to_1836(x,hadmed,0,startyear,endyear,numpy.float(-1.e30),numpy.float('nan'))
        ...
        hadens=numpy.zeros([len(enslist),hadmed.shape[1],x.shape[1]//2,x.shape[2]//2],dtype=numpy.float32)
        if ens!=0:
        ...
        if 'T.4' in prefix:
            ...
            numpy.savez('CRUT5_'+str(startyear)+'_'+str(endyear)+'.npz', hadmed=hadmed,hadtem=hadtem,hadens=hadens)
...
def rebin_20CRmonth(ps,fn):
            ...
            #mv='{:4}-{:0>2}-01'.format(year,month+1)
    t1=time.time()
    merra218=numpy.empty([1,16,1,18,36])
    merra272=numpy.empty([1,16,1,18*4,36*4])
    try:
        f=h5py.File(fn)
    except IOError:
        print((fn+' not found'))
        merra218.fill(numpy.nan)
        merra272.fill(numpy.nan)
        return merra272,merra218

    #l=(year-startyear)*12+month
    y=numpy.flip(f['t'][:],axis=0)
    #y[y>f.variables['ta']._FillValue/10.]=numpy.nan
    midx=numpy.zeros(16,dtype=int)
    for ip in range(16):
        midx[ip]=numpy.where(f['level'][:]==ps[ip])[0]

    #merra272=numpy.empty([1,16,1,72,144])
        #ini=False
    print(time.time()-t1)
...
def read_20CRv3(path,version,ps,startyear=1979,endyear=2017,ens=0):
    ...
    t1=time.time()
    try:
        f=numpy.load('20CRv3_'+str(startyear)+'_'+str(endyear)+'.npz')
        CR20v372=f["CR20v372"]
        CR20v318=f["CR20v318"]
    except:
        ...
        with Pool(40) as p:
            rlist=list(p.map(func,fns))
        rlist72=[]
        rlist18=[]
        for r in rlist:
            rlist72.append(r[0])
            rlist18.append(r[1])
        CR20v372=numpy.concatenate(rlist72,axis=2)
        CR20v318=numpy.concatenate(rlist18,axis=2)
        #merra272=numpy.reshape(merra272,merra272.shape[1:])		
        numpy.savez('20CRv3_'+str(startyear)+'_'+str(endyear)+'.npz',CR20v318=CR20v318,CR20v372=CR20v372)#,merra272=merra272)
    print(('read_20CRv3: ',time.time()-t1))
    CR20v372=numpy.reshape(CR20v372,CR20v372.shape[1:])		
    return CR20v372,CR20v318#,merra218
...
def make_corr(ipath,opath,statid):
    ...
    if True:
        ...
        iidx=numpy.where(dat<cutdate)[0]
        oidx=numpy.where(odat>=cutdate)[0]
        ndat=numpy.empty_like(dat,shape=(len(iidx)+len(oidx)))
        nrdict={
            'rasocorr':numpy.empty_like(f.variables['rasocorr'][:],shape=(2,16,len(iidx)+len(oidx)))}
        if 'rasobreak' in f.variables.keys():
            
            nrdict['rasobreak']=numpy.empty_like(f.variables['rasobreak'][:],shape=(2,16,len(iidx)+len(oidx)))
        ...
        #lw={'x':4,'y':2,'z':1}
        #for var in 'x','y','z':
            #cdict[var]=numpy.zeros((2,16,45000),numpy.float32)
            ...
            #plt.plot(numpy.arange(45000)/365.25+1900,cdict[var][0,5,:],label=var,linewidth=lw[var])
                            ...
                            if name =='rasocorrmon':
                                ...
                                for k in range(ndat.shape[0]):
                                    try:
                                        ...
                                        idx=numpy.where(mdays>ndat[k])[0][0]
                                ...
                                h[name][:]=nrdict[name]
                                idx=numpy.where(f.variables['montemp'][:]==-999.)
                ...
                for ens in d["ens"]:
                    ...
                    if len(d["ens"])==1:
                        ...
                        if "andepmon" in str(d["file"]):
                            #gn=''
                            gn=os.path.join(fpath,statid[sids:],d["msufile"]+d["suff"]+'bt1'+statid[sids:]+".nc")
                        else:
                            gn=os.path.join(fpath,statid[sids:],d["msufile"]+d["suff"]+'bt1'+statid[sids:]+".nc")
                    else:
                        if "ce20c" in d["file"]:
                            fn=os.path.join(fpath,statid[sids:],d["file"]+"{0:0>1}".format(ens)+d["suff"]+statid[sids:]+".nc")
                            gn='' #os.path.join(fpath,statid[sids:],d["msufile"]+"{0:0>1}".format(ens)+d["suff"]+'bt1'+statid[sids:]+".nc")
                        else:
                            fn=os.path.join(fpath,statid[sids:],d["file"]+d["suff"]+"{0:0>2}".format(ens)+'_'+statid[sids:]+".nc")
                            gn=os.path.join(fpath,statid[sids:],d["msufile"]+d["suff"]+"{0:0>2}".format(ens)+'bt1'+statid[sids:]+".nc")
                    ...
                    if len(d["index"])==1:
                        ...
                        if d['shortname']=='hadat':
                            ...
                            if '_rms' in d['shortname']:
                                ...
                                if 'seconds' in f.variables['datum'].getncattr('units'):
                                    s=d["data"][0].shape
                                    hilf=numpy.empty((1,s[0],s[1],s[2],d['ddata'][-1]))
                                    mstarts=calcdays(19000101,d['data'][istat].shape[3])
                                    hilf.fill(numpy.nan)
                                    index=dat
                                    copystride(hilf,numpy.array(f.variables[dv][:]),index,0,ens,pindex,nc_miss_val)
...
def loadsondes(path,tasks,days,plotproperties,stnames,init,slowload,satlist):
    ...
    if slowload:
        ...
        if len(llist)>0:
            istat,lats,lons,ps,stnames=read_alltrends(path,llist,days,satlist,lats,lons,ps,stnames,reftasks=tasks)
        if newall:
            llist=copy.deepcopy(tasks)
        for k in range(len(llist)):
            sd=dict()
...
def fill_suny(tasks,shortnames,stnames,lats,lons,ps,pindex):
    ...
    for s in ['suny','sunyhom']:
        try:
            ...
            if type(tasks[ishs[-1]]['data']) is list:
                tasks[ishs[-1]]['data']=numpy.empty([tasks[0]['data'].shape[0]]+tasks[ishs[-1]]['data'][1:],dtype=numpy.float32)
                tasks[ishs[-1]]['msudata']=numpy.empty([tasks[0]['msudata'].shape[0]]+tasks[ishs[-1]]['msudata'][1:],dtype=numpy.float32)
        except:
            pass
    ...
    with netCDF4.Dataset(ifname,'r') as f:
        siteids=numpy.asarray(f.variables['SiteID'][:].T.flatten().view('S11').astype('U11'))
        slats=numpy.array(f.variables['lat'])
        slons=numpy.array(f.variables['lon'])
        ipx=numpy.arange(16,dtype='int')
        ipx[15]=14
        itxy=(f.variables['time'][:]-190000)//100
        itym=(f.variables['time'][:]-190000)%100-1
        itx=numpy.array(itxy*12+itym)
        itxfinal=numpy.searchsorted(itx,tasks[ishs[0]]['data'].shape[4])
        idx=numpy.zeros(siteids.shape[0],dtype=int)-999
        for ish in ishs:
            
            if shortnames[ish]=='suny':
                T=numpy.array(f.variables['rawT'][:])
            elif shortnames[ish]=='sunyhom':
                T=numpy.array(f.variables['homoT'][:])
            else:
                continue
            T[T==-9999.]=numpy.nan
            j=0
            ...
            for i in range(siteids.shape[0]):            
                try:
                    idx[i]=numpy.where(stnames==siteids[i].rstrip()[-6:])[0][0]
                    #print(idx[i],stnames[idx[i]],siteids[i].rstrip()[-6:])
                    j+=1
                except:
                    idx[i]=tasks[ish]['data'].shape[0]

                if idx[i]==tasks[ish]['data'].shape[0]:
                    #print(i,siteids[i],idx[i])
                    #print('missed ID, trying lat/lon')
                    #print(slats[i],slons[i])
                    #print(numpy.where(numpy.logical_and(numpy.abs(slats[i]-lats)<1.0,numpy.abs(slons[i]-lons)<1.0)))
                    try:
                        
                        idx[i]=numpy.where(numpy.logical_and(numpy.abs(slats[i]-lats)<2.0,numpy.abs(slons[i]-lons)<2.0))[0][0]
                        j+=1
                    except:
                        jj+=1
                        pass
    ...
    if slowload:
        if init:
            ...
            if '20CRv3' in plotproperties['monthlytasks']:
                sats['20CRv3']['full'],sats['20CRv3']['18']=read_20CRv3(os.path.expandvars('$RSCRATCH/20CRv3/'),'',
                                                plotproperties['ps'],startyear=startyear,endyear=endyear,ens=0)
                sats['20CRv3']['18slopes']=numpy.zeros([1,16,hadmed.shape[2],hadmed.shape[3]],dtype=numpy.float32)
        ...
        if 'belts' in plotproperties['plotlist']:
            ...
            hadslopes=numpy.zeros([hadens.shape[0],hadmed.shape[2],hadmed.shape[3]],dtype=numpy.float32)
            first=(intervals[0]-startyear)*12
            last=(intervals[1]-startyear+1)*12
            if first<0:
                print((os.getcwd()+': Interval ',intervals,' not covered by available data starting at ',startyear))
...
#		expandandadd(tasks[key]["data"],currentdatabg,tasks[key]["index"],pindex,iens,currentdata,-1.0)
            ...
            elif tasks[key]["shortname"] in ['suny','sunyhom','rharm_h']: 
...
#		    tasks[key]["msudata"][cmask]=numpy.nan
                ...
                if ss[0]==sm[0] and ss[4]==sm[3]:
                    ...
                    if tasks[key]['shortname']=='tm':
                        tmindex=numpy.where(numpy.isnan(currentdatamsu))
                    else:
                        if tasks[key]['type']=='SAT':
                            currentdatamsu[tmindex]=numpy.nan
...
#                    anomalies=jcurrentdatamsu #numpy.copy(currentdatamsu[:])
                    anomalies=numpy.copy(currentdatamsu[:])
                    jpindex=msupindex
                                ...
                                for k in range(12):
                                    ...
                                    if numpy.sum(numpy.isnan(cg))<2:
                                        ...
                                        if ganomalies.shape[4]<=h.shape[1]:
                                            ...
                                            ganomalies[j,i,0,0,h.shape[1]+k::12]=numpy.nan
                                    else:
...
#		    tasks[key]['zslopes'][0,:,0]=zslopes[:,0]
                    ...
                    if ganomalies.shape[4]>10000 and jpindex.shape[0]>13 and 4 in jpindex:
                        ...
                        if "stations" in plotproperties["plotlist"]:
                            ...
                            with open(plotproperties['tmppath']+'/'+'stnames_'+tasks[key]["shortname"]+estr+'_'+
                                ...
                                for a,b,c in zip(stnames[mask][cind[:maxs]],
                                                 vec[mask][cind[:maxs]].astype(numpy.float32),
...
#					plot(out,projection,inp,symb,title,legend,coastlines)
                ...
                if 'belts' in plotproperties['plotlist']:
                    ...
                    hadzslopes=numpy.zeros([hadens.shape[0],nj,1])
...
#		    legend_user_lines=legend_user_lines+["HadCRUT4"]
    ...
    if 'belts' in plotproperties['plotlist']:
        ...
        hadbeltslopes=numpy.zeros([hadens.shape[0],belts.shape[0],1])
        try:
        ...
        for ibelt in range(len(beltnames)):
            ...
            amplevels=numpy.where(numpy.logical_and(ps[pindex]>150,ps[pindex]<=400))[0]
            print(('amplevels:',amplevels))
            hadbeltq=numpy.quantile(hadbeltslopes[:,ibelt,0],[0.05,0.5,0.95])
            for key in ti:
                if nodata[key]:
                    continue
                ...
                if legname not in [ "rio","rit", 'ce20c' ] or shade==0:
                        ...
                        if 'satseries' in plotproperties['plotlist']:
                            ...
                            try:
                                if tasks[key]['shortname'] in ['uah','rss','star']:
                                    ...
                                    ampl=amplevels.copy()
                                trmax=numpy.max(tasks[key]["beltslopes"][iens,ibelt,ampl].flatten())
                    ...
                    amp=numpy.zeros((tasks[key]['amps'].shape[0],tasks[key]["ens"].shape[0]))
                    if 'satseries' in plotproperties['plotlist']:
                        for iens in tasks[key]["ens"]: #range(len(tasks[key]["ens"])):
                        ...
                        for iens in tasks[key]["ens"]: #range(len(tasks[key]["ens"])):
                            try:
                                if tasks[key]['shortname'] in ['uah','rss','star']:
                                    ...
                                    ampl=amplevels.copy()
                                amp[:,iens]=numpy.max(tasks[key]["beltslopes"][iens,ibelt,ampl])/hadbeltq
                            except:
                                amp[:,iens]=numpy.nan
                    amps=" {0:3.1f}".format(numpy.mean(amp,axis=1)[1])
                    tasks[key]['amps'][:,:,interv,ibelt]=amp[:]
                    legend_user_lines=legend_user_lines+[legname+amps]

                tasks[key]['had'][:,interv,ibelt]=numpy.quantile(hadbeltslopes[:,ibelt,0],[0.05,0.5,0.95])
...
#        x, y = m(*numpy.meshgrid(gglons,glats))
        x, y = numpy.meshgrid(gglons,glats)
