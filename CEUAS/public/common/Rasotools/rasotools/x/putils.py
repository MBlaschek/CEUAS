...
#pythran export stationaverage(float[][][][], float)
def stationaverage(currentdata, thresh):

    s = currentdata.shape
    currentdataav=numpy.zeros((s[1],s[2],s[3]),dtype=numpy.float)
    avcount=numpy.zeros((s[1],s[2],s[3]),dtype=numpy.int32)
        ...
        for k in range(b.shape[2]):
            for m in range(pindex.shape[0]):
                for j in range(b.shape[4] - 1):
                    if index[l, j + 1] == 0:
                        break
                    a[l, 0, k, m, index[l, j]:index[l, j + 1]] = b[l, 0, k, pindex[m], j]
