...
def panomalies_and_slopes(si,series,startyear,interval,tolerance,iens,itime,anomalies,climatologies,good,slopes):
    ...
    sshape=series.shape
    climatology=numpy.zeros(12)
    anomaly=numpy.empty(sshape[3])
    orig=numpy.empty(sshape[3])
    stop=interval[1]
    start=interval[0]
#    for si in prange(sshape[0]):
...
def guanomalies_and_slopes(series,startyear,interval,tolerance,iens,itime,orig,anomaly,anomalies,climatology,good,slopes):
    ...
    gulinregress(numpy.asarray(itime,dtype='float64'),anomalies[:,:,:,itime],good,numpy.asarray([tolerance]),slopes[:,0,:2,:anomalies.shape[2]])
    slopes*=120
    return
    for si in range(sshape[0]):
        for ipar in range(sshape[1]):
...
def xyanomaliesmbb(field,fieldplus,fieldminus,syear,interval,anomaly,climatology,hilf,season):
    ...
    try:
        ...
        fieldplus[:]=numpy.nanmean(field,axis=0)
        ...
        for ipar in range(sh[2]):
            for ip in range(sh[3]):
                good=dailyanomaly(h[ist,ens,ipar,ip,:],numpy.int(d["startdate"]/10000),
                                  numpy.array(plotinterval),anomaly,climatology)
...
def pgrid_anomalies(anomaly,good,tolerance,gstatindex,ganomalies,gslopes,start,stop,itime,j):
    ...
    orig=numpy.empty(itime.shape[0])
    gst=numpy.empty(anomaly.shape[3],dtype=numpy.int32)
...
def tend(av):
    ...
    t=numpy.arange(144)/120.
    s=numpy.empty((12,av.shape[2],av.shape[3]))
    for im in range(12):
...
def globalfluxubdreh(vflux,spread=True):
    ...
    coslats=numpy.sin((-90.+numpy.arange(s[2]+1)*180./s[2])*math.pi/180.)
    deltacos=coslats[1:s[2]+1]-coslats[0:s[2]]
    coslats=numpy.cos((-90.+numpy.arange(s[2]+1)*180./s[2])*math.pi/180.)
...
def todreh(vflux):
    ...
    return zmflux-numpy.mean(zmflux)/3.
...
def tanfluxub(vflux,spread=True):
    ...
    erad=6370000.
    coslats=numpy.cos((-90.+numpy.arange(s[2]+1)*180./s[2])*math.pi/180.)
    sinlats=numpy.sin((-90.+numpy.arange(s[2]+1)*180./s[2])*math.pi/180.)
    mcoslats=0.5*(coslats[1:s[2]+1]+coslats[0:s[2]])
...
def calc_biases(bgdep,elev):
    X=numpy.zeros(bgdep.shape)
#     for it in range(1,bgdep.shape[3]):

    A=numpy.ones([2,2*bgdep.shape[0]])
    A[0,bgdep.shape]
    c=numpy.linalg.lstsq(A,bgdep[0,:,0,:])
