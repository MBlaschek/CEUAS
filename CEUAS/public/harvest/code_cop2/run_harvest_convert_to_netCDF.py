...
for d in datasets:
       ...
       if d not in ['era5_1', 'era5_1_mobile', 'era5_2', 'era5_2_mobile']:
              ...
              f_list = ['"' + db[d] + '/' + s.replace('.nc', '')+ '"' for s in f_list ] # adjust names with "" symbols and remove .nc extension
              ...
              if check_missing:
                     try:
                            processed = [s.split('_harvested_')[1].replace('.??????.','._').replace('.txt.gz.nc','') for s in os.listdir(processed_dir) if 'harvested' in s]
                            if d == 'era5_1':
