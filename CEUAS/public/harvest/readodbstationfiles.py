        ...
        if statid not in ['header','odbfile']:
            ...
            if len(s['wdidx'])>0:
                ...
                ugood=select_standardpressuredata(s['zref'],ps*100.,
                                                  s['mdatum'],s['mhours'],s['data'],ugood,tidx,s['wdidx'],windno,hl,                                                       
                                                  obs=s['uobsvalue'],
...
def select_standardpressuredata(zref,ps,mdatum,mhour,data,good,tidx,didx,varno,hl,
    ...
    m=-1
    idxold=-1
    for l in range(len(didx)): # didx: array([      0,       1,       2, ..., 3608401, 3608402, 3608403])
        i=didx[l]
        if l>0:
            im1=didx[l-1]
        else:
        ...
        # idx index of the numpy array
        if idxold<idx:
            m+=1
            idxold=idx
            
        if mhour[ihour,m]!= data[i,vhour]//10000: #mhour(2,9916)
            
            # data.shape is (3608404, 11)
            #if data[i][2] == 5.0: # check only pressure = 5
                #print('data[i][2] ' , data[i][2])
                #print('not equal!', mhour[ihour,m] , data[i,vhour] )
                #print('i',i , data[i][2])
                #print ('idxold, idx , m: ', idxold, idx, m )            
                #print ('ihour:',ihour,', vhour:',vhour,' , mhour:',mhour[:3],' , mhour[ihour,m]:',mhour[ihour,m],' , data[i,vhour]',data[i,vhour], ' , day:', data[0][0])
                #print ('continue')
            
            continue
        #print('They are equal!')
        #print('i',i)
        #print ('idxold, idx , m ', m, ihour, vhour, mhour[ihour,m] , data[i,vhour])            
        #print ('ihour:',ihour,'vhour:',vhour,'mhour:',mhour,'mhour[ihour,m]:',mhour[ihour,m],'data[i,vhour]',data[i,vhour], 'day:', data[0][0])
        #print ('continue')
...
def do_hours(tidx,mdatum,mhours,mtemperatures,data,sonde_type,hl,varnos):
    """ extracts the hours fo the observation,
        and converts to standard 0:00 and 12:00 time 
        
        If multiple observations are available,
        only the closest one (in time difference) wrt 0:00 and 12:00 is kept.
        
        Note that in the case of the 0:00 hour, the closes observation
        might come from the previous day. 
        
        Args: 
             data: np array of shape (:,11) where [:,0] is the date, [:,1] is the hour etc.
                   according to ['date','time','obstype','codetype','lat','lon','stalt','vertco_type','vertco_reference_1','varno','obsvalue']
    ...
    for i in range(data.shape[0]):   # here data is a np array with shape (:, 11)
        ...
        day=md%100
        hour=int(data[i,1])//10000
        #print('year, month, date, hour', year, month, day, hour )
        ...
        mtemperatures[ih,:,m]=0. # initializing the values to 0 when they should be filled (before it was nan)
        ...
        if mhours[ih,m]==-999: # mhours shape is (2,9916)
            ...
            if ih==1:
                ...
                #else:
                    #print('not closest')
        #print ('mdatum, mhours', mdatum, mhours)                
        if hl[7]!=-1:
...
For each day, this is a dictionary with three entries:
res[day]['12_h']   = -1
res[day]['00_h_m'] = -1
res[day]['00_h_n'] = -1

Then, each entry is analised.
In fact, the problem arises with the observations between 
0-6 and 18-24
when you have to compare the observation 18-24 of the current day,
and the observation 0-6 of the wolling day.

For each day, if the observation 0-6 falls closest to 00:00
than the observation 18-24 of the previous day, then it is kept,
and the hour is reported.
If it is not the closest, a -1 value is indicated, meaning that for the 
hour 00:00 of the current day, the hopur between 18-24 of the previous day
should be used.
"""
...
def do_hours_new(tidx,mdatum,mhours,mtemperatures,data,sonde_type,hl,varnos):
    ...
    def best_measurement_hour_singleday(day,hour,res):
        """ select the best measurement for each md, i.e.
            the measurement closest to 0 (0-6),
    ...
    m=-1
    ixold=-1
    """ Initialize the arrays (so that the entry exists even for i =0)
        delta_h0 contains the difference between the hour of the measurement and the 0:00 time
        delta_h12 contains the difference between the hour of the measurement and the 12:00 time
        day_0 is the day to be associated to the measurement closest to 0
        day_12 is the day to be associated to the measurement closest to 12 (trivial)
    ...
    for i in range(data.shape[0]):   # here data is a np array with shape (:, 11)
        ...
        #print('The date is, the hour is , value', md, hour )
        
        iday = best_measurement_hour_singleday(md,hour,res) # filling the res dictionary  
        
        
        
        
        ix=tidx[(year-1900)*12+month-1]+day-1+iday  #e.g. tidx array([    0,    31,    59, ..., 44833, 44864, 44894])  

        if ixold<ix:
            m+=1

        #mtemperatures[ih,:,m]=0. # initializing the values to 0 when they should be filled (before it was nan)
        #print('m, ix, ixold,iday', m, ix, ixold, iday)
        #mdatum[m]=ix
    ...
    #for h00, h12 in zip(clean['hour_12'], clean['hour_00'] ):
        #print('h00, h12: ',h00, h12)
        #input('check hours')
