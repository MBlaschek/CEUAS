...
class Merger():
      ...
      def make_obstab_era5fb_dic(self, dataset = '' , date_time = ''):                                                                                                                      
            ...
            for p,var,ind in zip ( obs_dic['z_coordinate'] , obs_dic['observed_variable'], range(len(obs_dic['z_coordinate'])) ):
            ...
            red_obs_dic = {}
            for v in self.observations_table_vars:
                  #red_obs_dic[v] = np.take( obs_dic[v],indices ) # does not work with array f lists, it flattens them 
                  red_obs_dic[v] = obs_dic[v][indices]

                  # if v == 'observation_id':
                  #       code.interact(local = locals()) 

                  #print('old' , v , ' ' , self.data[dataset]['observations_table'][v][index:index_up] , len(self.data[dataset]['observations_table'][v][index:index_up]) )
                  #print('new', obs_dic[v] , len(obs_dic[v]))
                  #input('check ')

            

            """ Need to remove duplicates: select only one combination for (pressure-observed_variable-observation_vale combination)"""
                                                                                                                                                                                            
            for v in self.era5fb_columns:                                                                                                                                                   
                  #era5fb_dic[v] = np.take(self.data[dataset]['era5fb_tab'][v][index:index_up] , indices )
                  era5fb_dic[v] = self.data[dataset]['era5fb_tab'][v][index:index_up][indices]

            # TODO fill empty era5fb / fill era5fb for the other datasets
            return red_obs_dic , era5fb_dic    
      ...
      def merge_all_data(self):       
            ...
            date_times=date_times[25000:27000]
            tot = len(date_times)
            tt=time.time()
            for dt, c in zip(date_times, range(tot) ): # loop over all the possible date_times 
                  ...
                  best_ds, combined_obs_tab, combined_era5fb_tab, combined_head_tab = self.combine_record(dt, container = cleaned_df_container)
      ...
      def combine_record(self, dt, container = ''):
            ...
            #""" If I have only one dataset, I simply return it """      
            #if len(container) == 1:
            ...
            if 'era5_2_1' in best_datasets and 'era5_1' not in best_datasets:
                  best_ds = 'era5_2_1'                  
            elif 'era5_2_2' in best_datasets and 'era5_1' not in best_datasets:
                  best_ds = 'era5_2_2'
            else:
                  best_ds = 'era5_1'
            ...
            for var in ['observation_id']:
                  if type (selected_obstab[var] ) == np.ndarray and type (selected_obstab[var][0] ) == np.bytes_:
                        selected_obstab[var] = np.array ([b''.join(l) for l in selected_obstab[var] ] )

            report_id = selected_obstab['report_id'][0]
            for var in ['report_id']:
                  if type (selected_obstab[var] ) == np.ndarray and type (selected_obstab[var][0] ) == np.bytes_:
                        value = b''.join(b''.join(report_id))  # it is the same for each row in the table
                        selected_obstab[var] = np.array( (1, len( selected_obstab[var]) ) ).fill(value)
            ...
            selected_head = self.get_header_table(dt, ds = best_ds )
            ...
            if 'era5_1_' in best_ds or 'era5_2' in best_ds:
            ...
            #code.interact(local = locals()) 
            return  best_ds, selected_obstab, selected_era5fb, selected_head
