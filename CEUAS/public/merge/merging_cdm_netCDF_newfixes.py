...
class Merger():
      ...
      def clean_dataframe(self, df_in , what = ''):  # not in use now 
      ...
      def get_reanalysis_feedback(self, dt, merged_observations_table, reanalysis = '' , best_ds = ''):
            ...
            if best_ds == 'era5_1' and reanalysis == 'era5fb':   # reading the feedback for era5_1 
                  ...
                  fb = self.data[best_ds][reanalysis]
                  fb =  self.data[best_ds][reanalysis].iloc[index_low:index_up]                 
      ...
      def merge_all_data(self):       
            ...
            for dt, c in zip(date_times, range(tot) ): # loop over all the possible date_times 
                  ...
                  merged_observations_table['source_id'] = best_ds                             
            ...
            #merged_obs = merged_obs.replace( -2147483648 , np.nan )
      ...
      def merge_record(self, dt, container = ''):
            ...
            #header = header.replace(-2147483648 , np.nan )
      ...
      def write_merged_file(self):
            ...
            for k in self.data.keys():                                                                                                                       
                  ...
                  sc.to_netcdf(out_name, format='netCDF4', encoding=encodings['station_configuration'] , engine='h5netcdf', mode='a' , group = group_name ) 

                  group_name = k + '_source_configuration'                                                                                                
                  sc = self.data[k]['source_configuration']                                                                                               
                  sc.to_netcdf(out_name, format='netCDF4', encoding=encodings['source_configuration'] , engine='h5netcdf', mode='a' , group = group_name )  
            ...
            for k in obs_tab.columns:
                  ...
                  write_dict_h5(out_name, df, 'observations_table', encodings['observations_table'], var_selection=[], mode='a', attrs = attrs_dic  ) ### TO DO !!! 
            ...
            head_tab = self.MergedHead            
            for v in head_tab.columns:   
                  if v == "index" or v == "hdrlen" or v == "string80":
                        continue            
                  #print('Writing the header table using h5py new method for the variable: ' , v )
                  ...
                  #except:
                  #      pass
                  #print( ' type of ', v , '   is    ',     type(a[v].values[0] ) )
                  write_dict_h5(out_name, a, 'header_table', encodings['header_table'], var_selection=[], mode='a', attrs = attrs_dic  ) ### TO DO !!!             
            ...
            for k in era5fb.columns:
                  ...
                  try:
                        write_dict_h5(out_name, df, 'era5fb', era5fb_encodings,  var_selection=[], mode='a' ) ### TO DO !!! 
                  except:
