      ...
      def merge_record(self, dt, container = ''):
            ...
            for k, v in record_dataset_legth.items():                 
                  ...
                  if v > 0:
                        ...
                        try: 
                              all_ds_reports.append( self.observation_ids_merged[k] * 1000000000  + int(b''.join(container[k]['df']['report_id'].values[0])) )  # converting the 
            ...
            try:
                  merged_report = self.observation_ids_merged[best_ds] * 10**(self.id_string_length-1)  + int(b''.join(container[k]['df']['report_id'].values[0]) )                  
            except:
            ...
            try: 
                  obs_ids_merged=[self.observation_ids_merged[best_ds]*10**(self.id_string_length-1)+ int(b''.join(i)) for i in selected_df['observation_id'].values ]
            except:
