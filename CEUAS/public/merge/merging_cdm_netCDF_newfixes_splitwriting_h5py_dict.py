...
def nobsfill(y,x,bds):
      ...
      #for i in range(y.shape[0]): 
            #y[i,0]=bds[0]
            ##l=0
            #y[i,y.shape[1]-x.shape[1]:]=x[i,:]
            ##for j in range(y.shape[1]-x.shape[1],y.shape[1]):
                  ##y[i,j]=x[i,l]
                  ##l=l+1
      return
...
class Merger():
      ...
      def initialize_data(self , station = '', datasets = {} ):
            ...
            for k,v in datasets.items() :  
                  ...
                  try:                       
                        ...
                        obst_tab = h5py_file['observations_table']
                  ...
                  #names = list(data[k]['header_table'].keys())
                  #formats=[data[k]['header_table'][i].dtype for i in data[k]['header_table'].keys()]
                  #dtype = dict(names = names, formats=formats)
                  #x=np.array(list(data[k]['header_table'].items()), dtype=dtype)

                  #data[k]['header_table'] = np.array(list(data[k]['header_table'].items()), dtype=dtype)
      ...
      def get_reanalysis_feedback(self, dt_bestds_dic, fbc, reanalysis = ''):
            """ Extracts the renanalysis feedback from the dt_bestds dictionary.
                  ...
                  #fb={}
                  #fb_columns=[]
                  #with h5py.File(self.datasets['era5_1'],mode='r') as f:
                        #for k in f['era5fb'].keys():
                              #if 'string' not in k and k!='index':  
                                    #fb_columns.append(k)
                                    #fb[k]=f['era5fb'][k][:]
            ...
            for dt, obstab in dt_bestds_dic.items():
                  ...
                  if dataset == 'era5_1' and reanalysis == 'era5fb':   # reading the feedback for era5_1 
                        ...
                        feedbacks.append(fb[index_low:index_up] )
                        #input('check FB', fb)
                        ...
                        if fb.ndim==2:
                              empty_fb=np.empty_like(fb[0:len_obstab,:])
                        else:
                              empty_fb = np.empty(len_obstab,dtype=fb[0].dtype) #pd.DataFrame(np.nan, columns= fb_columns ,  index = range(len_obstab) )                  
                        empty_fb.fill(np.nan)
                        feedbacks.append(empty_fb)

            merged_fb = np.concatenate(feedbacks)
            print(blue + 'Memory used after reading era5fb['+fbc+']: ', process.memory_info().rss/1000000000 , cend)

            return {fbc:merged_fb}
      ...
      def get_header_table(self , dt, ds = '' , all_ds = '', length = '', report_id = '' , obs_tab = '' ):
            ...
            hd = self.data[ds]['header_table'][index]
...
#            return{}
            ...
            hd['report_id'] = obs_tab['report_id'][0]
            source_id = ds.rjust(10) 
            hd['source_id'] = np.array([source_id],dtype='S10')[0]
            hd['report_timestamp'] = obs_tab['date_time'][0]

#            hd = frame #pd.DataFrame(frame)
...
#            hd['source_id'] = (hd['source_id']).astype('S10') 
      ...
      def make_obstab_df(self, dataset = '' , date_time = ''):
            """ Create a pandas df by reading the observations_table using h5py """
            #f = self.dataset[dataset]
            ...
            for v in self.observations_table_vars:
                  df_dic[v] = self.data[dataset]['observations_table'][v][index:index_up]
                  #print(obs_tab_h5py)
                  #df_dic[v] = obs_tab_h5py


            #print('FF DF DIC IS: ', df_dic)
      ...
      def merge_all_data(self):       
            ...
            for dt, c in zip(date_times, range(tot) ): # loop over all the possible date_times 
                  ...
                  for k in self.dataset_per_dt[dt] :  # checking the list of available datasets  
                        ...
                        chunk = self.make_obstab_df( dataset= k , date_time =dt)

                        if len(chunk['date_time'])==0:
                              continue
                  ...
                  merged_observations_table['source_id'] = np.asarray(merged_observations_table['source_id'],dtype='S10')
                  ...
                  header['report_id'] = merged_observations_table['report_id'][0]
                  all_merged_head.append(header)

                  """ New merged recordindex and recordtimestamps indices """
                  merged_indices.append(len(merged_observations_table['date_time']))                 
            ...
            merged_obs = {}
            for k in all_merged_obs[0].keys():                        
                  merged_obs[k]=np.concatenate([all_merged_obs[i][k][:] for i in range(len(all_merged_obs))])
                  self.write_merged(content = 'observations_table', table= {k:merged_obs[k]})
                  if k not in ['date_time']:
                        del merged_obs[k]

            self.write_merged(content = 'observations_table', table= {})
...
#            dateindex = np.array( [ i // 86400 for i in merged_obs['date_time'] ] )            
            dateindex = merged_obs['date_time']//86400           
            ...
            x=np.concatenate([all_merged_head])
            l=0
            for k in self.data['igra2']['header_table'].dtype.names:
                  merged_hd[k]=np.array([x[i][k] for i in range(x.shape[0])])# np.concatenate([all_merged_head])
                  l+=1
...
#            merged_hd = pd.concat (all_merged_head)
            ...
            del merged_hd , all_merged_head,x
            ...
            with h5py.File(self.datasets['era5_1'],mode='r') as f:
                  for k in f['era5fb'].keys():
                        if 'string' not in k and k!='index':  
                              fb_columns.append(k)
            for fbc in fb_columns:          
                  fb_tab = self.get_reanalysis_feedback(dt_bestds_dic, fbc, reanalysis = 'era5fb')            
      ...
      def merge_record(self, dt, container = ''):
            ...
            for k, v in record_dataset_legth.items():                 
                  ...
                  if v > 0:
                        ...
                        try: 
                              bds=np.array([chr(48+self.observation_ids_merged[k])],dtype='S1')
                              c=np.array([[b'00000000000000']]).view('S1')
                              #cs=14-container[k]['df']['report_id'][0].shape[0]
                              #c[cs:]=container[k]['df']['report_id'][0]
                              #c[0]=bds
                              nobsfill(c,container[k]['df']['report_id'],bds)
                              all_ds_reports[k]=c.view('S14')[0]  # converting the 
                              #all_ds_reports.append( self.observation_ids_merged[k] * 1000000000  + int(b''.join(container[k]['df']['report_id'][0])) )  # converting the 
                              ...
                              #all_ds_reports.append( self.observation_ids_merged[k] * 1000000000  + int(container[k]['df']['report_id'].values[0]) )  # converting the                  
                              #a = int(b''.join(container[k]['df']['report_id'].values[0]))
                              #a = b''.join(container[k]['df']['report_id'].values[0] )
                              #print('FF as string ', container[k]['df']['report_id'].values[0] )
                              #print('FF as string ', a , int(a), 'INT ISSSS ' )

                              #print('FF as string ', int(container[k]['df']['report_id'].values[0]) )
                              ...
                              #print(int( b''.join(container[k]['df']['report_id'][0] ) ).decode('utf-8') )
                              input('check wrong report_id !!! FF ')
            ...
            selected_df = copy.copy(container[best_ds]['df'])  # might take extra time, dont know how to get rid of this 

            clen=len(container[k]['df']['report_id'])
            try:
#                  merged_report = self.observation_ids_merged[best_ds] * 10**(self.id_string_length-1)  + int(b''.join(container[k]['df']['report_id'][0]) )                  
#                  bds=np.bytes_('{}'.format(self.observation_ids_merged[best_ds]))
                  merged_report = np.array([all_ds_reports[best_ds]]*clen).astype('S14').reshape(clen)              
            except:
                  #rint(container[k]['df']['report_id'].values[0])
            ...
            selected_df['observation_id'] = obs_ids_merged.view('S14').reshape(obs_ids_merged.shape[0])
            ...
            selected_df['report_id'] = merged_report #np.bytes_(merged_report).reshape(len(merged_report))
            ...
            if len(all_ds_reports) > 1: 
                  x=b",".join( all_ds_reports.values() )
                  duplicates =   np.array([x],dtype='S{}'.format(len(x)))[0]
            else:
                  duplicates = next(iter(all_ds_reports.values()))[0] #str(all_ds_reports[0])
            ...
            if best_ds == 'era5_1':  
                  selected_df['advanced_assimilation_feedback'] = [1]*len(selected_df['date_time'])
            else:
                  selected_df['advanced_assimilation_feedback'] = [1]*len(selected_df['date_time'])
...
#                  obs_tab = self.add_cdm_missing_columns(obs_tab)     
                  ...
                  for k in obs_tab.keys():
                        if k == 'index' :
                              continue
                        if type(obs_tab[k][0]) != self.variable_types['observations_table'][k]:
                  ...
                  #print('Writing the observation table using h5py new method for the variable: ' , k )
                        df = {k:obs_tab[k]}  # making a 1 column dataframe  
                        print('writing',k)
                  ...
                  if obs_tab=={}:
                        with h5py.File(out_name,'r') as f:
                              obstabkeys=list(f['observations_table'].keys())
                              shp=f['observations_table']['date_time'].shape
                        for k in self.encodings['observations_table'].keys():
                              if self.variable_types['observations_table'][k] == np.int32 :
                                    nan = np.int32(-2147483648)
                              else:
                                    nan = np.float32(np.nan) 
      
                              if k not in obstabkeys:
                                    logging.debug('Adding missing cdm colum with empty values: %s' , k )
      
                                    df={k:np.empty(shp,dtype=np.dtype(nan))}
                                    df[k].fill(nan)
                                    print('write empty',k)
                                    write_dict_h5(out_name, df, 'observations_table', self.encodings['observations_table'],
                                                  var_selection=[], mode='a', attrs = attrs_dic  ) ### TO DO !!! 
                  ...
                  for k in table.keys():
                        ...
                        try:
                              write_dict_h5(out_name, table, 'era5fb', self.era5fb_encodings,  var_selection=[], mode='a' ) ### TO DO !!! 
                        except:
                              print('Could not write the ' , k , ' column of the era5fb! ')
...
out_dir = '/raid60/scratch/leo/scratch/era5/odbs/m0/'
...
data_directories = {  'era5_1'       :  '/raid60/scratch/leo/scratch/era5/odbs/1/era5_1'       ,
                      'era5_1759' : '/raid60/scratch/federico/netCDF_converted_Jan2020/ready_for_merging/era5_1759'   ,
                                  'era5_1761' : '/raid60/scratch/federico/netCDF_converted_Jan2020/ready_for_merging/era5_1761'   ,
                                  'era5_3188' : '/raid60/scratch/federico/netCDF_converted_Jan2020/ready_for_merging/era5_3188'   ,     
...
if __name__ == '__main__':
      ...
      for stat in stations:
            ...
            if stat in processed_stations:
                  ...
                  os.remove(out_dir+stat+'_CEUAS_merged_v0.nc')
#                  continue
