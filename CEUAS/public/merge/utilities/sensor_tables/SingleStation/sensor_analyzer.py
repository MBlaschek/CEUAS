...
class Analyze():
    ...
    def __init__(self,Sensor, station):
...
class Plot():
    ...
    def time_series(self, data_df, label = ''):
        ...
        if self.save:
            plotly.offline.plot(fig, filename=  'plots/' + self.station + "_time_series.html" )
            pio.write_image(fig, "plots/" + self.station + "_timeSeries_ku.png")
    ...
    def sensor_table(self, data):
        ...
        fig.update_layout(width=1900, height=45*len(data))
...
def wrapper(station):
    ...
    # Analyze data
    analyze = Analyze(sensor,station)
    ...
    series = plot.time_series( data_df_clean_all, label='CIAO')
