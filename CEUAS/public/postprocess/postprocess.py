...
class MonthlyAverages(MergedFile):
    ...
    def make_monthly_observations_table(self, variables = ''):
        ...
        def get_observations(v, obs_var, z_coord_type, z_coord, obs_val,  indices = '' , era5fb = False , era5fb_departures = '' , era5obs = '' ):
            ...
            if era5fb:
                cdm_to_era5 = {85: 2} # converting observed variable from cdm to edb standards 
                v = cdm_to_era5[v]
            ...
            for i in range(len(indices[0]) ):
                ...
                for p in self.MergedFile.std_plevs:
                    ...
                    if 'observation_values' not in results[p].keys():
                        results[p]['observation_values']              = []
                        
                    if 'observation_indices' not in results[p].keys():    
                        results[p]['observation_indices']                 = []
                        
                    if era5fb:
                        if 'observation_values_era5fb' not in  results[p].keys():
                            results[p]['observation_values_era5fb']  = []  
                            
                        if 'era5fb_departures' not in  results[p].keys():
                            results[p]['era5fb_departures']               = []             
                    ...
                    if p not in list(plevels):
                        ...
                        if era5fb:
                            results[p]['era5fb_departures']              .append(np.nan)        
                            results[p]['observation_values_era5fb'] .append(np.nan)        
        ...
        def fill_columns(monthly_observations_table , tot_observations , unavailable_columns = '' , other_columns = '', index_min = '', indices = '' , dt = '', fill_obs = False  ):
            ...
            monthly_observations_table['secondary_value']  .extend   ( [np.nan] * tot_observations ) 
            if fill_obs:
                monthly_observations_table['observation_value']           .extend ( [np.nan] * tot_observations ) 
                monthly_observations_table['observation_value_global'].extend ( [np.nan] * tot_observations ) 
                monthly_observations_table['original_precision_global'] .extend ( [np.nan] * tot_observations )  
                monthly_observations_table['secondary_value_global']  .extend ( [np.nan] * tot_observations )            
                monthly_observations_table['observation_value_reanalysis'].extend ( [np.nan] * tot_observations ) 
                monthly_observations_table['original_precision_reanalysis'] .extend ( [np.nan] * tot_observations )  
                monthly_observations_table['secondary_value_reanalysis']  .extend ( [np.nan] * tot_observations )                   
            # ['observed_variable' , 'observation_value', 'z_coordinate' , 'z_coordinate_type', 'secondary_value' , 'value_significance' , 'original_precision', 'date_time'] 
        ...
        def get_all_obs_per_month( monthly_dates, variable= 85 ):
            """ Extract all the values for all the months, pressure levels and time.
                  Will be used to calculate the global statistics. """
                        ...
                        for time in ['00','12'] :
                            ...
                            if indices:  
                                ...
                                extracted_era5fb , index_min, index_max = get_observations(variable, obs_var_era5, z_coord_type, z_coord_era5, obs_val, indices = ind , 
                                                                                                                                   era5fb=True , era5fb_departures = era5fb_departures , era5obs = obs_era5)      
        ...
        for v in variables:
            ...
            monthly_observations_table['observation_value_global'] = []
            monthly_observations_table['original_precision_global']  = []
            monthly_observations_table['secondary_value_global']   = []   
            
            monthly_observations_table['observation_value_reanalysis'] = []
            monthly_observations_table['original_precision_reanalysis']  = []
            monthly_observations_table['secondary_value_reanalysis']   = []            
                ...
                for time in ['00','12'] :
                    ...
                    if not indices: # if I have no indices, I fill every column with nans
                        ...
                        if extracted_data['is_there_data']: # check if I have at least one valid observation, otherwise do nothing 
                            ...
                            dummy = fill_columns(monthly_observations_table , tot_observations , 
                            ...
                            for p in self.MergedFile.std_plevs:  # (self.MergedFile.std_plevs to keep same order ### extracted_data.keys()
                                ...
                                if values:
                                    ...
                                    if extracted_era5fb['is_there_data']:
                                        observations, departures, observations_era5 = extracted_era5fb[p]['observation_values'], extracted_era5fb[p]['era5fb_departures'], extracted_era5fb[p]['observation_values_era5fb']
                                        ...
                                        values_departures = [] 
                                        for o,d,f in zip(observations, departures, observations_era5):
