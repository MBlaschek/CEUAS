...
class MonthlyAverages(MergedFile):
    ...
    def write_monthly_file(self , monthly_tab):
        ...
        if len(monthly_tab['date_time']) ==0:
            out_file.replace('VARIABLE.nc', 'empty.dat') 
            a = open(out_file , 'w' )
                                    ...
                                    if extracted_era5fb['is_there_data']:
                                        ...
                                        for o,d,f,b in zip(observations, departures, observations_era5, biases ):
                                            if o == f and not np.isnan(o) and not np.isnan(d):
                                                ...
                                                if not np.isnan(b):
                                                    unbiased = o - b  # subtract the bias from the observations
                                                    values_departures_unbiased.append(unbiased) 
...
if __name__ == '__main__':
            ...
            merged_directory = '/raid60/scratch/federico/DATABASE_standard_plevels_OCTOBER2020/'

            """ Moving postprocessed files to new directory """
            postprocessed_new = '/raid60/scratch/federico/MONTHLY_AVERAGE_27NOV_biascorrection'
            os.system('mkdir ' + postprocessed_new)
