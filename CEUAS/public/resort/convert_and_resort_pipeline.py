...
def load_20CRoffset(readict, wpath):
       
    #from scipy.interpolate import RectBivariateSpline
    start = 1940
    refs = {}
    # try:
    #     with open(os.path.expandvars(wpath+'/refs{}x.pkl'.format(start)),'rb') as f:
    #         refs=pickle.load(f)
    # except:
...
def convert_missing(refs, pd_list, gactor, wpath,cyear, fn,
    ...
    readictorig={'era5':{'ftype':('t','fct'),'param':{'t':'130','u':'131','v':'132','q':'133','z':'129'},
                     'path':os.path.expandvars(f'{working_dir}/data/era5_1_data/'),'prefix':{'t':'era5t','fct': 'era5fc.0.25t'},'suffix':'','glue':'.'},
             #'CERA20C':{'ftype':('t',),'param':{'t':'130','u':'131','v':'132','q':'133','z':'129'},
               ...
               #'path':os.path.expandvars('$RSCRATCH/JRA55/split/'),'prefix':'test.','suffix':'grb','glue':'.'},
                        '20CRv3':{'ftype':('t',),'param':{'t':'TMP','u':'UGRD','v':'VGRD'},#,'q':'SPFH'},#,'z':'HGT'},
                         'path':os.path.expandvars(f'{working_dir}/data/20CRv3/'),'prefix':{'t': 'anl_meant' },'suffix':'_pres','glue':'_'},
               }
...
### code for checking if z_coordinate_type=2 exists in particular year
    ...
    inventory_list = [f'{working_dir}/code/station_configuration/CUON_station_configuration_extended.csv']
...
#        os.remove(logfile)
    ...
    #                print(fn, cyear, 'year missing in obs records')
        ...
        fmatch = glob.glob(os.path.expandvars(f'{working_dir}/harvest/harvest_regular/igra2/')+\
        ...
        with eua.CDMDataset(fn) as data:
           ...
           # loading data:
            ...
            path_to_gridded=os.path.expandvars(working_dir+'/data/era5_1_data/')
            try:
        ...
        try:
            ...
            ov_vars = np.empty(out['date_time'].shape[0], dtype='S21')
            ov_vars[:] = ''
                    ...
                    with eua.CDMDataset(fn) as data:
                        for i in obskeys:
                            ...
                            if i == 'index' :
                                ...
                                #dum = np.array(np.arange(cyear*100000000000000+99000000000000, cyear*100000000000000+99000000000000+dlen), dtype = 'S18')
                                dum=[f'{cyear}9900000{i:0>10d}' for i in np.arange(dlen)]
                                dum=np.array(dum, dtype='S21')
                                #dum = [dum_i[]
                                sh = dum.shape
                                dum = dum.view('S1').reshape(sh[0], 21)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Resort the netcdf files and add missing variables.")
    parser.add_argument('--inp_dir' , '-i', 
                    help="Directory where the files to resort are."  ,
                    type = str,
                    default = '.')
    ...
    parser.add_argument('--ceuas_dir' , '-c', 
                    help="Directory where to find the scripts."  ,
                    type = str,
                    default = '.')

    # parser.add_argument('--rscratch' , '-r', 
    #                 help="Directory where to find the meta data."  ,
    #                 type = str, 
    #                 default = '.')
    ...
    out_dir = args.working_dir + "/resort/"
    # rscratch = args.rscratch
    ceuas_dir = args.ceuas_dir
    
    sys.path.append(f'{ceuas_dir}/public/harvest/code/')
    from harvest_convert_to_netCDF import write_dict_h5

    sys.path.append(f'{ceuas_dir}/public/cds-backend/code/')
    import cds_eua4 as eua

    # sys.path.append(f'{ceuas_dir}/public/resort/rasotools-master/')
    # import rasotools
    # dir(rasotools)
    ...
    readict={'era5':{'ftype':('t','fct'),'param':{'t':'130','u':'131','v':'132','q':'133','z':'129'},
                     'path':os.path.expandvars(f'{working_dir}/data/era5_1_data/'),'prefix':'era5','suffix':'','glue':'.'},
             #'CERA20C':{'ftype':('t',),'param':{'t':'130','u':'131','v':'132','q':'133','z':'129'},
               #'path':os.path.expandvars('$RSCRATCH/CERA20C/'),'prefix':'CERA20C','suffix':'','glue':'.'},
                          #'JRA55':{'ftype':('fcst_mdl',),'param':{'t':'011_tmp','u':'033_ugrd','v':'034_vgrd','q':'051_spfh'},
               #'path':os.path.expandvars('$RSCRATCH/JRA55/split/'),'prefix':'test.','suffix':'grb','glue':'.'},
                        '20CRv3':{'ftype':('',),'param':{'t':'TMP','u':'UGRD','v':'VGRD'},#,'q':'SPFH'},#,'z':'HGT'},
                         'path':os.path.expandvars(f'{working_dir}/data/20CRv3/'),'prefix':'anl_meant','suffix':'_pres','glue':'_'},
               }
    ...
    files = glob.glob(f'{inp_dir}/*/*v3.nc')[:]
    files += glob.glob(f'{inp_dir}/_mobile/*/*v3.nc')[:]
    files += glob.glob(f'{inp_dir}/_orphan/*/*v3.nc')[:]
...
#    ray.init(address="localhost:6379")
    ...
    wpath = out_dir
    refs = load_20CRoffset(readict, out_dir)
    
    inventory_list = glob.glob(f'{working_dir}/code/station_configuration/CUON_station_configuration_extended.csv')
    ...
    if finput == 'yearly':
        ...
        drift = True
        futures = []
        for iyear in range(2025, 2024, -1): ## swtich
    ...
    futures =[] 
    ## SKIPPING the creation of LONG files.  
    # for fks in fkeys:
