...
def write_dict_h5(dfile, f, k, fbencodings, var_selection=[], mode='a', attrs={}): 
    ...
    with h5py.File(dfile,mode) as fd:
        ...
        for v in var_selection:
            # del fd[k][v] # replacing the old data -> need to del first EDIT
            if v in fd[k].keys():
                continue
...
@ray.remote
def redo_file(file, attrs_encodings, outdir = '/data/public/converted_v19'):
    import hdf5plugin
    print(file)
    targetfile = outdir + '/' + file.split('/')[-1].replace("_uncertainty", "")
    if os.path.isfile('./done_5/' + file.split('/')[-1] + '.txt') or os.path.isfile('./failed_5/' + file.split('/')[-1] + '.txt'):
        return
    # try: 
    file_path = './done_5/' + file.split('/')[-1] + '.txt'

    with h5py.File(file, 'r') as fi:
        table = 'advanced_uncertainty'
        for var in ['desroziers_30']:
            data = fi[table][var][:]
            with h5py.File(targetfile, 'r+') as to_edit:
                try:
                    to_edit[table][var][:] = data
                    with open(file_path, "w") as text_file:
                        text_file.write('done')
                except:
                    try:
                        if var in to_edit['table'].keys():
                            del to_edit[table][var]
                        attrs = {}
                        for var in ['30']:
...
files = glob.glob('/data/public/new_deso/*.nc') # data/public/converted_v19_irregular/*.nc')
attrs_encodings = np.load('dic_type_attributes.npy', allow_pickle=True).item()

#redo_file(files[0], attrs_encodings)

ray.init(num_cpus=15)# , object_store_memory=(10**9)*65)

#files = glob.glob('/data/public/converted_v19_irregular/*.nc')
#print(files)
result_ids = []
for file in files:
