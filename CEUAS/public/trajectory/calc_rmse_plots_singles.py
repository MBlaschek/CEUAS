...
#         maplt.title(str(year)+' Temperature RMSE \n' + str(len(results)) + ' stations    ' +str(len(rms_sum_shdisp[50000])) +' valid ascents')
    maplt.title(str(year)+' Temperature RMSE '+str(sid)+' \n' +str(len(rms_sum_shdisp[50000])) +' valid ascents')
    maplt.savefig('./rmse_singles/'+str(sid)+'_'+str(year)+'_world_rmse_plot_'+str(compare_to)+'.png')
    maplt.close()
...
#     year = 1990
    ...
    diff = True
    show_date = False
    for stat in ['91413', '50527','11035','70219', '08508','72357','10393']: 
        for year in [1970, 1980, 1990, 2000, 2010, 2020]: # 1960]:#, 
            file_list = []
            for i in glob.glob('/mnt/users/scratch/leo/scratch/converted_v9/*'+stat+'*.nc')[:20]:
                print(i)
