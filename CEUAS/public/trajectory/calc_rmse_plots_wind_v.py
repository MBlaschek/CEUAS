        ...
        with h5py.File(conv_file, 'r') as file:
            ...
            if len(idx) == 0:
                ...
                return [rmse_sum_shbase_sonde, rmse_sum_shbase_adjsonde, rmse_sum_shdisp_sonde,
                        ...
                        rms_sum_shdisp, rms_sum_dispminusbase, stat_marker, dt_marker]
...
#             print(day, pd.Timestamp(ds_fc_time.time.values))
            ...
            for i in range(len(stdplevs)):
                if np.abs(stdplevs[i] - find_nearest(input_data.z_coordinate,stdplevs[i])) > 500:
                    ...
                    dt_marker[stdplevs[i]].append(day)
    ...
    with open('rmse_v_out/era5_v_fc_'+str(year)+'_'+str(sid)+'_rmse_data.p', 'wb') as file:
        pickle.dump(
...
#     year = 1990
    ...
    for year in [2000]: #[1960, 1970, 1980, 1990, 2000, 2010, 2020]:
        ...
        # for i in glob.glob('/mnt/users/scratch/leo/scratch/converted_v9/*04018*.nc')[:]:
        #     print(i)
        #     sid = i.split('_CEUAS_merged_v1.nc')[0][-5:]
        #     file_list.append(calc_station.remote(sid,year))
...
#             ax1.set_xlabel('eastward wind speed RMSE')
...
#             maplt.title(str(year)+' Eastward Wind Speed RMSE \n' +str(len(rms_sum_shdisp[50000])) +' valid ascents')
#             maplt.tight_layout()
#             maplt.savefig(str(year)+'_era5_fc_outlier_cleaned_world_rmse_plot_u.png')
# #             maplt.show()
#             maplt.close()
