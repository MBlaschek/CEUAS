...
def calc_station(sid, year, selected_mons = None):
    ...
    #     display(df_mon)
        ...
        if compare_to == 'fc':
            ...
            files = glob.glob('/mnt/scratch/scratch/leo/scratch/era5/gridded/era5t.'+str(year)+str(mon).zfill(2)+'*.131.nc')[0]
...
#     year = 1990
    ...
    for year in [2000, 2010, 2020]: # [1960, 1970, 1980, 1990, 2000, 2010, 2020]:
        ...
        ax1.set_xlabel('eastward windspeed RMSE')
...
#         maplt.title(str(year)+' Temperature RMSE \n' + str(len(results)) + ' stations    ' +str(len(rms_sum_shdisp[50000])) +' valid ascents')
        maplt.title(str(year)+' Eastward Windspeed RMSE \n' +str(len(rms_sum_shdisp[50000])) +' valid ascents')
        maplt.savefig(str(year)+'_u_era5_fc_world_rmse_plot_igra.png')
        maplt.close()
