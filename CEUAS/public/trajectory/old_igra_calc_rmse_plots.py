...
#     year = 1990
    ...
    diff = True
    show_date = False
    for var in ['air temperature']:#, 'eastward windspeed', 'northward windspeed', 'specific humidity']: # ['eastward windspeed', 'northward windspeed', 'air temperature', 'specific humidity']
        for year in [2000]: # [1960, 1970, 1980, 1990, 2000, 2010, 2020]:
            ...
            # with open('original_era5_' + save_dict[var] + '_fc_'+str(year)+'_rmse_data.p', 'wb') as file:
            ...
            # with open('/users/staff/uvoggenberger/CEUAS/CEUAS/public/trajectory/original_era5_' + save_dict[var] + '_fc_'+str(year)+'_rmse_data.p', 'rb') as file:
            ...
            #     targetvar = np.array(rmse_sum_shbase_sonde[stdplevs[i]])
            #     pc_shbs_plev = np.nanpercentile(targetvar, [1,99])
            #     targetvar = targetvar[np.logical_and(targetvar < pc_shbs_plev[1], targetvar > pc_shbs_plev[0])]
            #     rmse_shbase_sonde.append(np.sqrt(np.nanmean((np.array(targetvar)**2))))
            #     if show_date:    
            #         print('rmse_shbase_sonde - plev: ', stdplevs[i], ' RMSE: ', rmse_shbase_sonde[-1])

            #     targetvar = np.array(rmse_sum_shdisp_sonde[stdplevs[i]])
            #     pc_shbs_plev = np.nanpercentile(targetvar, [1,99])
            #     targetvar = targetvar[np.logical_and(targetvar < pc_shbs_plev[1], targetvar > pc_shbs_plev[0])]
            #     rmse_shdisp_sonde.append(np.sqrt(np.nanmean((np.array(targetvar)**2))))
            #     if show_date:    
            #         print('rmse_shdisp_sonde - plev: ', stdplevs[i], ' RMSE: ', rmse_shdisp_sonde[-1])

            #     targetvar = np.array(rms_sum_shbase[stdplevs[i]])
            #     pc_shbs_plev = np.nanpercentile(targetvar, [1,99])
            #     targetvar = targetvar[np.logical_and(targetvar < pc_shbs_plev[1], targetvar > pc_shbs_plev[0])]
            #     rms_shbase.append(np.sqrt(np.nanmean((np.array(targetvar)**2))))
            #     if show_date:    
            #         print('rms_shbase - plev: ', stdplevs[i], ' RMS: ', rms_shbase[-1])

            #     targetvar = np.array(rms_sum_sonde[stdplevs[i]])
            #     pc_shbs_plev = np.nanpercentile(targetvar, [1,99])
            #     targetvar = targetvar[np.logical_and(targetvar < pc_shbs_plev[1], targetvar > pc_shbs_plev[0])]
            #     rms_sonde.append(np.sqrt(np.nanmean((np.array(targetvar)**2))))
            #     if show_date:    
            #         print('rms_sonde - plev: ', stdplevs[i], ' RMS: ', rms_sonde[-1])

            #     targetvar = np.array(rms_sum_shdisp[stdplevs[i]])
            #     pc_shbs_plev = np.nanpercentile(targetvar, [1,99])
            #     targetvar = targetvar[np.logical_and(targetvar < pc_shbs_plev[1], targetvar > pc_shbs_plev[0])]
            #     rms_shdisp.append(np.sqrt(np.nanmean((np.array(targetvar)**2))))
            #     if show_date:
            #         print('rms_shdisp - plev: ', stdplevs[i], ' RMS: ', rms_shdisp[-1])

            #     targetvar = np.array(rms_sum_dispminusbase[stdplevs[i]])
            #     pc_shbs_plev = np.nanpercentile(targetvar, [1,99])
            #     targetvar = targetvar[np.logical_and(targetvar < pc_shbs_plev[1], targetvar > pc_shbs_plev[0])]
            #     rms_dispmbase.append(np.sqrt(np.nanmean((np.array(targetvar)**2))))
            #     if show_date:
            #         print('rms_dispmbase - plev: ', stdplevs[i], ' RMS: ', rms_shdisp[-1])
            ...
            # maplt.savefig(str(year)+'_'+save_dict[var]+'_era5_fc_world_rmse_plot_cuon_log_bgtc.png')
            # maplt.close()
